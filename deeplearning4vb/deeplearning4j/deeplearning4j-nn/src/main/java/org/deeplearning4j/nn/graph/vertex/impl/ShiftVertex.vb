...
Namespace org.deeplearning4j.nn.graph.vertex.impl
	...
	Public Class ShiftVertex
		...
		Private shiftFactor As Double

		Public Sub New(ByVal graph As ComputationGraph, ByVal name As String, ByVal vertexIndex As Integer, ByVal shiftFactor As Double, ByVal dataType As DataType)
			Me.New(graph, name, vertexIndex, Nothing, Nothing, shiftFactor, dataType)
		End Sub

		Public Sub New(ByVal graph As ComputationGraph, ByVal name As String, ByVal vertexIndex As Integer, ByVal inputVertices() As VertexIndices, ByVal outputVertices() As VertexIndices, ByVal shiftFactor As Double, ByVal dataType As DataType)
		...
		Public Overrides Function doForward(ByVal training As Boolean, ByVal workspaceMgr As LayerWorkspaceMgr) As INDArray
			If Not canDoForward() Then
				Throw New System.InvalidOperationException("Cannot do forward pass: inputs not set (ShiftVertex " & vertexName_Conflict & " idx " & vertexIndex_Conflict & ")")
			End If

			If inputs.Length > 1 Then
				Throw New System.ArgumentException("ShiftVertex (name " & vertexName_Conflict & " idx " & vertexIndex_Conflict & ") only supports 1 input.")
			End If
		...
		Public Overrides Function doBackward(ByVal tbptt As Boolean, ByVal workspaceMgr As LayerWorkspaceMgr) As Pair(Of Gradient, INDArray())
			If Not canDoBackward() Then
				Throw New System.InvalidOperationException("Cannot do backward pass: errors not set (ShiftVertex " & vertexName_Conflict & " idx " & vertexIndex_Conflict & ")")
			End If
		...
		Public Overrides WriteOnly Property BackpropGradientsViewArray As INDArray
			Set(ByVal backpropGradientsViewArray As INDArray)
				If backpropGradientsViewArray IsNot Nothing Then
					Throw New Exception("Vertex does not have gradients; gradients view array cannot be set here (ShiftVertex " & vertexName_Conflict & " idx " & vertexIndex_Conflict & ")")
				End If
			End Set
		End Property

		Public Overrides Function ToString() As String
			Return "ShiftVertex(id=" & Me.VertexIndex & ",name=""" & Me.VertexName & """,shiftFactor=" & shiftFactor & ")"
		End Function
