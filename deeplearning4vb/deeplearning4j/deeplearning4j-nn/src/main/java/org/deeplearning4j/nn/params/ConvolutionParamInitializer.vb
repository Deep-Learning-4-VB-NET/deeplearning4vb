...
'JAVA TO VB CONVERTER NOTE: The field INSTANCE was renamed since Visual Basic does not allow fields to have the same name as other class members:
		Private Shared ReadOnly INSTANCE_Conflict As New ConvolutionParamInitializer()

		Public Shared ReadOnly Property Instance As ConvolutionParamInitializer
			Get
		...
		Public Overridable Function numParams(ByVal l As Layer) As Long Implements ParamInitializer.numParams
			Dim layerConf As ConvolutionLayer = DirectCast(l, ConvolutionLayer)

			Dim kernel() As Integer = layerConf.getKernelSize()
			Dim nIn As val = layerConf.getNIn()
			Dim nOut As val = layerConf.getNOut()
			Return nIn * nOut * kernel(0) * kernel(1) + (If(layerConf.hasBias(), nOut, 0))
		End Function

		Public Overridable Function paramKeys(ByVal layer As Layer) As IList(Of String)
			Dim layerConf As ConvolutionLayer = DirectCast(layer, ConvolutionLayer)
		...
		Public Overridable Function init(ByVal conf As NeuralNetConfiguration, ByVal paramsView As INDArray, ByVal initializeParams As Boolean) As IDictionary(Of String, INDArray)
			Dim layer As ConvolutionLayer = CType(conf.getLayer(), ConvolutionLayer)
			...
			Dim layerConf As ConvolutionLayer = CType(conf.getLayer(), ConvolutionLayer)
		...
		Public Overridable Function getGradientsFromFlattened(ByVal conf As NeuralNetConfiguration, ByVal gradientView As INDArray) As IDictionary(Of String, INDArray)
			...
			If layerConf.hasBias() Then
				...
				Dim weightGradientView As INDArray = gradientView.get(NDArrayIndex.interval(0,0,True), NDArrayIndex.interval(nOut, numParams(conf))).reshape("c"c, nOut, nIn, kernel(0), kernel(1))
				[out](BIAS_KEY) = biasGradientView
				[out](WEIGHT_KEY) = weightGradientView
			Else
				Dim weightGradientView As INDArray = gradientView.reshape("c"c, nOut, nIn, kernel(0), kernel(1))
				[out](WEIGHT_KEY) = weightGradientView
			End If
			Return [out]
		End Function
		...
		Protected Friend Overridable Function createWeightMatrix(ByVal conf As NeuralNetConfiguration, ByVal weightView As INDArray, ByVal initializeParams As Boolean) As INDArray
	'        
	...
	'         
			...
			If initializeParams Then
				...
				Dim fanOut As Double = outputDepth * kernel(0) * kernel(1) / (CDbl(stride(0)) * stride(1))

				Dim weightsShape As val = New Long() {outputDepth, inputDepth, kernel(0), kernel(1)}

				Return layerConf.getWeightInitFn().init(fanIn, fanOut, weightsShape, "c"c, weightView)
			Else
				Dim kernel() As Integer = layerConf.getKernelSize()
				Return WeightInitUtil.reshapeWeights(New Long() {layerConf.getNOut(), layerConf.getNIn(), kernel(0), kernel(1)}, weightView, "c"c)
			End If
		End Function
	End Class
