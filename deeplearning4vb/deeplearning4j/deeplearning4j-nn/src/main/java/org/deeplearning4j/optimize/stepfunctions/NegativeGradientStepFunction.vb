Imports System
Imports Slf4j = lombok.extern.slf4j.Slf4j
Imports StepFunction = org.deeplearning4j.optimize.api.StepFunction
Imports INDArray = org.nd4j.linalg.api.ndarray.INDArray

'
' *  ******************************************************************************
' *  *
' *  *
' *  * This program and the accompanying materials are made available under the
' *  * terms of the Apache License, Version 2.0 which is available at
' *  * https://www.apache.org/licenses/LICENSE-2.0.
' *  *
' *  *  See the NOTICE file distributed with this work for additional
' *  *  information regarding copyright ownership.
' *  * Unless required by applicable law or agreed to in writing, software
' *  * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
' *  * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
' *  * License for the specific language governing permissions and limitations
' *  * under the License.
' *  *
' *  * SPDX-License-Identifier: Apache-2.0
' *  *****************************************************************************
' 

Namespace org.deeplearning4j.optimize.stepfunctions

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Slf4j public class NegativeGradientStepFunction implements org.deeplearning4j.optimize.api.StepFunction
	<Serializable>
	Public Class NegativeGradientStepFunction
		Implements StepFunction

		Public Overridable Sub [step](ByVal x As INDArray, ByVal line As INDArray, ByVal [step] As Double) Implements StepFunction.step
			Me.step(x, line)
		End Sub

		Public Overridable Sub [step](ByVal x As INDArray, ByVal line As INDArray) Implements StepFunction.step
			x.subi(line)
		End Sub

		Public Overridable Sub [step]() Implements StepFunction.step

		End Sub
	End Class

End Namespace