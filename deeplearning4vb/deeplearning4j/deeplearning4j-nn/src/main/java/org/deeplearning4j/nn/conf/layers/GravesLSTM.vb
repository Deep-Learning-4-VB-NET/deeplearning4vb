...
'ORIGINAL LINE: @Deprecated @Data @NoArgsConstructor @ToString(callSuper = true) @EqualsAndHashCode(callSuper = true) public class GravesLSTM extends AbstractLSTM
	...
	Public Class GravesLSTM
		...
		Protected Friend Overrides Sub initializeConstraints(Of T1)(ByVal builder As org.deeplearning4j.nn.conf.layers.Layer.Builder(Of T1))
			...
			If DirectCast(builder, Builder).recurrentConstraints IsNot Nothing Then
				...
				For Each c As LayerConstraint In (DirectCast(builder, Builder)).recurrentConstraints
					...
					c2.Params = Collections.singleton(GravesLSTMParamInitializer.RECURRENT_WEIGHT_KEY)
					constraints.Add(c2)
				Next c
			End If
		End Sub
		...
		Public Overrides Function instantiate(ByVal conf As NeuralNetConfiguration, ByVal trainingListeners As ICollection(Of TrainingListener), ByVal layerIndex As Integer, ByVal layerParamsView As INDArray, ByVal initializeParams As Boolean, ByVal networkDataType As DataType) As Layer
			...
			Dim ret As New org.deeplearning4j.nn.layers.recurrent.GravesLSTM(conf, networkDataType)
		...
		Public Overrides Function initializer() As ParamInitializer
			Return GravesLSTMParamInitializer.Instance
		End Function
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @AllArgsConstructor public static class Builder extends AbstractLSTM.Builder<Builder>
		Public Class Builder
			Inherits AbstractLSTM.Builder(Of Builder)

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @SuppressWarnings("unchecked") public GravesLSTM build()
			Public Overridable Function build() As GravesLSTM
				Return New GravesLSTM(Me)
			End Function
		End Class
