	...
	Public Class Checkpoint
		...
		Public Shared Function fromFileString(ByVal str As String) As Checkpoint
			...
			If split.Length <> 6 Then
				Throw New System.InvalidOperationException("Cannot parse checkpoint entry: expected 6 entries, got " & split.Length & " - values = " & Arrays.toString(split))
			End If
			Return New Checkpoint(Integer.Parse(split(0)), Long.Parse(split(1)), Integer.Parse(split(2)), Integer.Parse(split(3)), split(4), split(5))
		End Function
