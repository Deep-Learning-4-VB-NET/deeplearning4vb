...
'JAVA TO VB CONVERTER NOTE: The field INSTANCE was renamed since Visual Basic does not allow fields to have the same name as other class members:
		Private Shared ReadOnly INSTANCE_Conflict As New Convolution3DParamInitializer()

		Public Shared ReadOnly Property Instance As Convolution3DParamInitializer
			Get
		...
		Public Overrides Function numParams(ByVal l As Layer) As Long
			Dim layerConf As Convolution3D = DirectCast(l, Convolution3D)
		...
		Public Overrides Function init(ByVal conf As NeuralNetConfiguration, ByVal paramsView As INDArray, ByVal initializeParams As Boolean) As IDictionary(Of String, INDArray)
			Dim layer As Convolution3D = CType(conf.getLayer(), Convolution3D)
			...
			Dim layerConf As Convolution3D = CType(conf.getLayer(), Convolution3D)
			Dim nOut As val = layerConf.getNOut()
		...
		Public Overrides Function getGradientsFromFlattened(ByVal conf As NeuralNetConfiguration, ByVal gradientView As INDArray) As IDictionary(Of String, INDArray)
			...
			If layerConf.hasBias() Then
				...
				Dim weightGradientView As INDArray = gradientView.get(NDArrayIndex.interval(0,0,True), NDArrayIndex.interval(nOut, numParams(conf))).reshape("c"c, nOut, nIn, kernel(0), kernel(1), kernel(2))
				[out](BIAS_KEY) = biasGradientView
				[out](WEIGHT_KEY) = weightGradientView
			Else
				Dim weightGradientView As INDArray = gradientView.reshape("c"c, nOut, nIn, kernel(0), kernel(1), kernel(2))
				[out](WEIGHT_KEY) = weightGradientView
			End If
			Return [out]
		End Function
	...
	'         
			...
			If initializeParams Then
				...
				Dim weightsShape As val = New Long(){outputDepth, inputDepth, kernel(0), kernel(1), kernel(2)}

				Return layerConf.getWeightInitFn().init(fanIn, fanOut, weightsShape, "c"c, weightView)
			Else
				Dim kernel() As Integer = layerConf.getKernelSize()
				Return WeightInitUtil.reshapeWeights(New Long(){layerConf.getNOut(), layerConf.getNIn(), kernel(0), kernel(1), kernel(2)}, weightView, "c"c)
			End If
		End Function
	End Class
