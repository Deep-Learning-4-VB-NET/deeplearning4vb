...
'JAVA TO VB CONVERTER NOTE: The field INSTANCE was renamed since Visual Basic does not allow fields to have the same name as other class members:
		Private Shared ReadOnly INSTANCE_Conflict As New GravesLSTMParamInitializer()

		Public Shared ReadOnly Property Instance As GravesLSTMParamInitializer
			Get
		...
		Public Const RECURRENT_WEIGHT_KEY As String = LSTMParamInitializer.RECURRENT_WEIGHT_KEY
		Public Const BIAS_KEY As String = LSTMParamInitializer.BIAS_KEY
		Public Const INPUT_WEIGHT_KEY As String = LSTMParamInitializer.INPUT_WEIGHT_KEY
		...
		Public Overridable Function numParams(ByVal l As Layer) As Long Implements ParamInitializer.numParams
			Dim layerConf As org.deeplearning4j.nn.conf.layers.GravesLSTM = DirectCast(l, org.deeplearning4j.nn.conf.layers.GravesLSTM)
			...
			Dim nParams As val = nLast * (4 * nL) + nL * (4 * nL + 3) + 4 * nL 'bias

			Return nParams
		End Function

		Public Overridable Function paramKeys(ByVal layer As Layer) As IList(Of String)
			Return New List(Of String) From {INPUT_WEIGHT_KEY, RECURRENT_WEIGHT_KEY, BIAS_KEY}
		End Function

		Public Overridable Function weightKeys(ByVal layer As Layer) As IList(Of String)
			Return New List(Of String) From {INPUT_WEIGHT_KEY, RECURRENT_WEIGHT_KEY}
		End Function
		...
		Public Overridable Function init(ByVal conf As NeuralNetConfiguration, ByVal paramsView As INDArray, ByVal initializeParams As Boolean) As IDictionary(Of String, INDArray)
			...
			Dim layerConf As org.deeplearning4j.nn.conf.layers.GravesLSTM = CType(conf.getLayer(), org.deeplearning4j.nn.conf.layers.GravesLSTM)
	...
	'             
			...
			Else
				...
				params(RECURRENT_WEIGHT_KEY) = WeightInitUtil.reshapeWeights(New Long() {nL, 4 * nL + 3}, recurrentWeightView)
				params(BIAS_KEY) = biasView
			End If
		...
		Public Overridable Function getGradientsFromFlattened(ByVal conf As NeuralNetConfiguration, ByVal gradientView As INDArray) As IDictionary(Of String, INDArray)
			...
			Dim recurrentWeightGradView As INDArray = gradientView.get(NDArrayIndex.interval(0,0,True), NDArrayIndex.interval(nParamsIn, nParamsIn + nParamsRecurrent)).reshape("f"c, nL, 4 * nL + 3)
