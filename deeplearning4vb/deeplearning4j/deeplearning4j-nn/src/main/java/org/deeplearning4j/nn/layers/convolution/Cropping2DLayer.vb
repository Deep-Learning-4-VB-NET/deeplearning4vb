...
Namespace org.deeplearning4j.nn.layers.convolution
	...
	<Serializable>
	Public Class Cropping2DLayer
		Inherits AbstractLayer(Of org.deeplearning4j.nn.conf.layers.convolutional.Cropping2D)

		Private cropping() As Integer '[padTop, padBottom, padLeft, padRight]

		Public Sub New(ByVal conf As NeuralNetConfiguration, ByVal dataType As DataType)
			MyBase.New(conf, dataType)
			Me.cropping = CType(conf.getLayer(), org.deeplearning4j.nn.conf.layers.convolutional.Cropping2D).getCropping()
		End Sub
		...
		Private Function inputSubset(ByVal from As INDArray) As INDArray
			...
			If nchw Then
				'NCHW format
				Return from.get(all(), all(), interval(cropping(0), from.size(2) - cropping(1)), interval(cropping(2), from.size(3) - cropping(3)))
			Else
				'NHWC
				Return from.get(all(), interval(cropping(0), from.size(1) - cropping(1)), interval(cropping(2), from.size(2) - cropping(3)), all())
			End If
		End Function
	End Class
