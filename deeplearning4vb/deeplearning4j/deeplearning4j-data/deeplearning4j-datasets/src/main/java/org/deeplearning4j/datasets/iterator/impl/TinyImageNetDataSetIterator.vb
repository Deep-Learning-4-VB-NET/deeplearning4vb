...
Namespace org.deeplearning4j.datasets.iterator.impl
	...
	<Serializable>
	Public Class TinyImageNetDataSetIterator
...
'ORIGINAL LINE: @Getter protected org.nd4j.linalg.dataset.api.DataSetPreProcessor preProcessor;
		...
		Public Sub New(ByVal batchSize As Integer, ByVal imgDim() As Integer, ByVal set As DataSetType, ByVal imageTransform As ImageTransform, ByVal rngSeed As Long)
			MyBase.New((New TinyImageNetFetcher()).getRecordReader(rngSeed, imgDim, set, imageTransform), batchSize, 1, TinyImageNetFetcher.NUM_LABELS)
		End Sub
		...
		Public Shared Overloads Function getLabels(ByVal categories As Boolean) As IList(Of String)
			Dim rawLabels As IList(Of String) = (New TinyImageNetDataSetIterator(1)).getLabels()
			If categories Then
				Return rawLabels
			End If

			'Otherwise, convert to human-readable format, using 'words.txt' file
			Dim baseDir As File = DL4JResources.getDirectory(ResourceType.DATASET, TinyImageNetFetcher.LOCAL_CACHE_NAME)
			Dim labelFile As New File(baseDir, TinyImageNetFetcher.WORDS_FILENAME)
