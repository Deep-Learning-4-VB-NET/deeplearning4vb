...
'ORIGINAL LINE: Dim dat[][] As Integer = new Integer[Rows][Cols]
		...
		''' (i.e., same bits, but requires conversion before use) </summary>
		''' <param name="nImages"> Number of images </param>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public byte[][] readImagesUnsafe(int nImages) throws java.io.IOException
		Public Overridable Function readImagesUnsafe(ByVal nImages As Integer) As SByte()()
'JAVA TO VB CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
'ORIGINAL LINE: Dim out[][] As SByte = new SByte[nImages][0]
			Dim [out]()() As SByte = RectangularArrays.RectangularSByteArray(nImages, 0)
			For i As Integer = 0 To nImages - 1
				[out](i) = New SByte((rows_Conflict * cols_Conflict) - 1){}
				read([out](i))
			Next i
			Return [out]
		End Function
