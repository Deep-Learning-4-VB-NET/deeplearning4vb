...
'ORIGINAL LINE: @DisplayName("Eval Json Test") @NativeTag @Tag(TagNames.EVAL_METRICS) @Tag(TagNames.JACKSON_SERDE) @Tag(TagNames.TRAINING) @Tag(TagNames.DL4J_OLD_API) class EvalJsonTest extends org.deeplearning4j.BaseDL4JTest
	Friend Class EvalJsonTest
		Inherits BaseDL4JTest

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Serde Empty") void testSerdeEmpty()
		Friend Overridable Sub testSerdeEmpty()
			Dim print As Boolean = False
			Dim arr() As org.nd4j.evaluation.IEvaluation = {
				New Evaluation(),
			...
			For Each e As org.nd4j.evaluation.IEvaluation In arr
				Dim json As String = e.toJson()
				Dim stats As String = e.stats()
				If print Then
					Console.WriteLine(e.GetType() & vbLf & json & vbLf & vbLf)
				End If
				Dim fromJson As IEvaluation = CType(org.nd4j.evaluation.BaseEvaluation.fromJson(json, GetType(org.nd4j.evaluation.BaseEvaluation)), IEvaluation)
				assertEquals(e.toJson(), fromJson.toJson())
			Next e
		End Sub

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Serde") void testSerde()
		Friend Overridable Sub testSerde()
			Dim print As Boolean = False
			Nd4j.Random.setSeed(12345)
'JAVA TO VB CONVERTER NOTE: The variable evaluation was renamed since it may cause conflicts with calls to static members of the user-defined type with this name:
			Dim evaluation_Conflict As New Evaluation()
'JAVA TO VB CONVERTER NOTE: The variable evaluationBinary was renamed since it may cause conflicts with calls to static members of the user-defined type with this name:
			Dim evaluationBinary_Conflict As New EvaluationBinary()
			...
			Dim arr() As org.nd4j.evaluation.IEvaluation = { evaluation_Conflict, evaluationBinary_Conflict, roc, roc2, roc3, regressionEvaluation, ec }
			...
			evaluation_Conflict.eval(evalLabel, evalProb)
			...
			evaluationBinary_Conflict.eval(evalLabel, evalProb)
			...
			For Each e As org.nd4j.evaluation.IEvaluation In arr
				...
				If print Then
					Console.WriteLine(e.GetType() & vbLf & json & vbLf & vbLf)
				End If
				Dim fromJson As IEvaluation = CType(BaseEvaluation.fromJson(json, GetType(org.nd4j.evaluation.BaseEvaluation)), IEvaluation)
				assertEquals(e.toJson(), fromJson.toJson())
			Next e
		End Sub

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Serde Exact Roc") void testSerdeExactRoc()
		Friend Overridable Sub testSerdeExactRoc()
			Nd4j.Random.setSeed(12345)
			Dim print As Boolean = False
			Dim roc As New ROC(0)
			Dim roc2 As New ROCBinary(0)
			Dim roc3 As New ROCMultiClass(0)
			Dim arr() As org.nd4j.evaluation.IEvaluation = { roc, roc2, roc3 }
			...
			For Each e As org.nd4j.evaluation.IEvaluation In arr
				...
				End If
				Dim fromJson As org.nd4j.evaluation.IEvaluation = BaseEvaluation.fromJson(json, GetType(org.nd4j.evaluation.BaseEvaluation))
				...
				ElseIf TypeOf e Is ROCBinary Then
					Dim rocs() As org.nd4j.evaluation.classification.ROC = CType(fromJson, ROCBinary).getUnderlying()
					Dim origRocs() As org.nd4j.evaluation.classification.ROC = CType(e, ROCBinary).getUnderlying()
					' for(ROC r : rocs ){
					For i As Integer = 0 To origRocs.Length - 1
						Dim r As org.nd4j.evaluation.classification.ROC = rocs(i)
						Dim origR As org.nd4j.evaluation.classification.ROC = origRocs(i)
				...
				ElseIf TypeOf e Is ROCMultiClass Then
					Dim rocs() As org.nd4j.evaluation.classification.ROC = CType(fromJson, ROCMultiClass).getUnderlying()
					Dim origRocs() As org.nd4j.evaluation.classification.ROC = CType(e, ROCMultiClass).getUnderlying()
					For i As Integer = 0 To origRocs.Length - 1
						Dim r As org.nd4j.evaluation.classification.ROC = rocs(i)
						Dim origR As org.nd4j.evaluation.classification.ROC = origRocs(i)
						' Shouldn't have probAndLabel, but should have stored AUC and AUPRC, AND stored curves
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Json Yaml Curves") void testJsonYamlCurves()
		Friend Overridable Sub testJsonYamlCurves()
			Dim roc As New ROC(0)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Json With Custom Threshold") void testJsonWithCustomThreshold()
		Friend Overridable Sub testJsonWithCustomThreshold()
