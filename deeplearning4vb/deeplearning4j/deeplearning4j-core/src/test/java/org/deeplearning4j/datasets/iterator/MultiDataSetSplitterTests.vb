...
Imports MultiDataSetGenerator = org.deeplearning4j.datasets.iterator.tools.MultiDataSetGenerator
...
'ORIGINAL LINE: @NativeTag @Tag(TagNames.FILE_IO) public class MultiDataSetSplitterTests extends org.deeplearning4j.BaseDL4JTest
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Public Overridable Sub testSplitter_1()
			Dim back As val = New MultiDataSetGenerator(1000, New Integer(){32, 100}, New Integer(){32, 5})

			Dim splitter As val = New MultiDataSetIteratorSplitter(back, 1000, 0.7)
			...
			For e As Integer = 0 To numEpochs - 1
				...
				Do While train.hasNext()
					Dim data As val = train.next().getFeatures(0)
...
'ORIGINAL LINE: assertEquals((float) cnt++, data.getFloat(0), 1e-5,"Train failed on iteration " + cnt + "; epoch: " + e);
				...
				Do While test.hasNext()
					Dim data As val = test.next().getFeatures(0)
'JAVA TO VB CONVERTER TODO TASK: The following line contains an assignment within expression that was not extracted by Java to VB Converter:
'ORIGINAL LINE: assertEquals((float) cnt++, data.getFloat(0), 1e-5,"Train failed on iteration " + cnt + "; epoch: " + e);
					assertEquals(CSng(cnt)++, data.getFloat(0), 1e-5,"Train failed on iteration " & cnt & "; epoch: " & e)
					gcntTest += 1
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test public void testMultiSplitter_1()
		Public Overridable Sub testMultiSplitter_1()
			Dim back As val = New MultiDataSetGenerator(1000, New Integer(){32, 100}, New Integer(){32, 5})

			' we're going to mimic train+test+validation split
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){800, 100, 100})
			...
			For e As Integer = 0 To numEpochs - 1
				...
				Do While trainIter.hasNext()
					...
					Dim i As Integer = 0
					Do While i < ds.getFeatures().length
						assertEquals(CDbl(globalIter), ds.getFeatures()(i).getDouble(0), 1e-5f,"Failed at iteration [" & globalIter & "]")
						i += 1
					Loop
					globalIter += 1
				Loop
...
'ORIGINAL LINE: @Test public void testSplitter_5()
		Public Overridable Sub testSplitter_5()
			...
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){900, 100})

			Dim iteratorList As IList(Of MultiDataSetIterator) = splitter.getIterators()
			Dim numEpochs As val = 10
				...
				For Each partIterator As val In iteratorList
					...
					Do While partIterator.hasNext()
						...
						For i As Integer = 0 To data.length - 1
							assertEquals(CSng(perEpoch), data(i).getFloat(0), 1e-5,"Train failed on iteration " & cnt & "; epoch: " & e)
						Next i
						'gcntTrain++;
						[global] += 1
						cnt += 1
...
'ORIGINAL LINE: @Test public void testUnorderedSplitter_1()
		Public Overridable Sub testUnorderedSplitter_1()
			...
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){500, 500})
...
'JAVA TO VB CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
					...
					For i As Integer = 0 To data.length - 1
						assertEquals(CSng(farCnt), data(i).getFloat(0), 1e-5,"Train failed on iteration " & cnt & "; epoch: " & e)
					Next i
					cnt += 1
					[global] += 1
				Loop
...
'JAVA TO VB CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
					...
					For i As Integer = 0 To data.length - 1
'JAVA TO VB CONVERTER TODO TASK: The following line contains an assignment within expression that was not extracted by Java to VB Converter:
'ORIGINAL LINE: assertEquals((float) cnt++, data[i].getFloat(0), 1e-5,"Train failed on iteration " + cnt + "; epoch: " + e);
						assertEquals(CSng(cnt)++, data(i).getFloat(0), 1e-5,"Train failed on iteration " & cnt & "; epoch: " & e)
					Next i
					[global] += 1
				Loop
			Next e
...
'ORIGINAL LINE: @Test public void testUnorderedSplitter_2()
		Public Overridable Sub testUnorderedSplitter_2()
			...
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){2})
...
'JAVA TO VB CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
					Dim data As val = iteratorList(partNumber).next().getFeatures()
					For i As Integer = 0 To data.length - 1
						assertEquals(CSng(500 * partNumber + cnt), data(i).getFloat(0), 1e-5,"Train failed on iteration " & cnt)
					Next i
					cnt += 1
				Loop
			Next partNumber
...
'ORIGINAL LINE: @Test public void testUnorderedSplitter_3()
		Public Overridable Sub testUnorderedSplitter_3()
			...
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){10})
...
'ORIGINAL LINE: @Test public void testUnorderedSplitter_4()
		Public Overridable Sub testUnorderedSplitter_4()
			...
			' we're going to mimic train+test+validation split
			Dim splitter As val = New MultiDataSetIteratorSplitter(back, New Integer(){80, 10, 5})
			...
			Do While validationIter.hasNext()
				...
				Dim i As Integer = 0
				Do While i < ds.getFeatures().length
					assertEquals(CSng(valCnt) + 90, ds.getFeatures()(i).getFloat(0), 1e-5,"Validation failed on iteration " & valCnt)
					i += 1
				Loop
				valCnt += 1
			Loop
