...
Namespace org.deeplearning4j.graph.iterator
	...
	Public Class RandomWalkIterator(Of V)
...
'ORIGINAL LINE: public RandomWalkIterator(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength)
		Public Sub New(ByVal graph As IGraph(Of T1), ByVal walkLength As Integer)
		...
		''' <seealso cref= #RandomWalkIterator(IGraph, int, long, NoEdgeHandling) </seealso>
'JAVA TO VB CONVERTER TODO TASK: Wildcard generics in constructor parameters are not converted. Move the generic type parameter and constraint to the class header:
'ORIGINAL LINE: public RandomWalkIterator(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength, long rngSeed)
		Public Sub New(ByVal graph As IGraph(Of T1), ByVal walkLength As Integer, ByVal rngSeed As Long)
...
'JAVA TO VB CONVERTER TODO TASK: Wildcard generics in constructor parameters are not converted. Move the generic type parameter and constraint to the class header:
'ORIGINAL LINE: public RandomWalkIterator(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength, long rngSeed, org.deeplearning4j.graph.api.NoEdgeHandling mode)
		Public Sub New(ByVal graph As IGraph(Of T1), ByVal walkLength As Integer, ByVal rngSeed As Long, ByVal mode As NoEdgeHandling)
...
'JAVA TO VB CONVERTER TODO TASK: Wildcard generics in constructor parameters are not converted. Move the generic type parameter and constraint to the class header:
'ORIGINAL LINE: public RandomWalkIterator(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength, long rngSeed, org.deeplearning4j.graph.api.NoEdgeHandling mode, int firstVertex, int lastVertex)
		Public Sub New(ByVal graph As IGraph(Of T1), ByVal walkLength As Integer, ByVal rngSeed As Long, ByVal mode As NoEdgeHandling, ByVal firstVertex As Integer, ByVal lastVertex As Integer)
			Me.graph = graph
...
'JAVA TO VB CONVERTER NOTE: The local variable next was renamed since Visual Basic will not allow local variables with the same name as their enclosing function or property:
			Dim next_Conflict As Vertex(Of V)
			Try
				next_Conflict = graph.getRandomConnectedVertex(currVertexIdx, rng)
			Catch e As NoEdgesException
				Select Case mode
					Case NoEdgeHandling.SELF_LOOP_ON_DISCONNECTED
						For i As Integer = 1 To walkLength_Conflict - 1
				...
				End Select
			End Try
			indices(1) = next_Conflict.vertexID()
			currVertexIdx = indices(1)

			For i As Integer = 2 To walkLength_Conflict '<= walk length: i.e., if walk length = 2, it contains 3 vertices etc
				next_Conflict = graph.getRandomConnectedVertex(currVertexIdx, rng)
				currVertexIdx = next_Conflict.vertexID()
				indices(i) = currVertexIdx
			Next i
			Return New VertexSequence(Of V)(graph, indices)
		End Function
