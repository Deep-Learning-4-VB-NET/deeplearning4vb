...
Namespace org.deeplearning4j.graph.iterator.parallel
	...
	Public Class RandomWalkGraphIteratorProvider(Of V)
		Implements GraphWalkIteratorProvider(Of V)

'JAVA TO VB CONVERTER WARNING: Java wildcard generics have no direct equivalent in VB:
'ORIGINAL LINE: private org.deeplearning4j.graph.api.IGraph<V, ?> graph;
		Private graph As IGraph(Of V, Object)
...
'ORIGINAL LINE: public RandomWalkGraphIteratorProvider(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength)
...
'JAVA TO VB CONVERTER TODO TASK: Wildcard generics in constructor parameters are not converted. Move the generic type parameter and constraint to the class header:
'ORIGINAL LINE: public RandomWalkGraphIteratorProvider(org.deeplearning4j.graph.api.IGraph<V, ?> graph, int walkLength, long seed, org.deeplearning4j.graph.api.NoEdgeHandling mode)
		Public Sub New(ByVal graph As IGraph(Of T1), ByVal walkLength As Integer, ByVal seed As Long, ByVal mode As NoEdgeHandling)
			Me.graph = graph
		...
		Public Overridable Function getGraphWalkIterators(ByVal numIterators As Integer) As IList(Of GraphWalkIterator(Of V)) Implements GraphWalkIteratorProvider(Of V).getGraphWalkIterators
			...
			For i As Integer = 0 To numIterators - 1
				...
				Dim iter As GraphWalkIterator(Of V) = New RandomWalkIterator(Of V)(graph, walkLength, rng.nextLong(), mode, from, [to])
				list.Add(iter)
				last = [to]
			Next i
