...
Namespace org.deeplearning4j.graph.data.impl
	...
	Public Class WeightedEdgeLineProcessor
		Implements EdgeLineProcessor(Of Double)
		...
		Public Overridable Function processLine(ByVal line As String) As Edge(Of Double) Implements EdgeLineProcessor(Of Double).processLine
			...
			If split.Length <> 3 Then
				Throw New System.ArgumentException("Invalid line: expected format """ & 0 & delimiter & 1 & delimiter & "weight""; received """ & line & """")
			End If

			Dim from As Integer = Integer.Parse(split(0))
			Dim [to] As Integer = Integer.Parse(split(1))
			Dim weight As Double = Double.Parse(split(2))
			Return New Edge(Of Double)(from, [to], weight, directed)
		End Function
	End Class
