...
Imports KerasCropping1D = org.deeplearning4j.nn.modelimport.keras.layers.convolutional.KerasCropping1D
...
'ORIGINAL LINE: @DisplayName("Keras Cropping 1 D Test") @Tag(TagNames.FILE_IO) @Tag(TagNames.KERAS) @NativeTag class KerasCropping1DTest extends org.deeplearning4j.BaseDL4JTest
	Friend Class KerasCropping1DTest
		Inherits BaseDL4JTest

		Private ReadOnly LAYER_NAME As String = "cropping_1D_layer"

		Private ReadOnly CROPPING As Integer = 2
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test @DisplayName("Test Cropping 1 D Layer") void testCropping1DLayer() throws Exception
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Friend Overridable Sub testCropping1DLayer()
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: private void buildCroppingSingleDim1DLayer(org.deeplearning4j.nn.modelimport.keras.config.KerasLayerConfiguration conf, System.Nullable<Integer> kerasVersion) throws Exception
		Private Sub buildCroppingSingleDim1DLayer(ByVal conf As KerasLayerConfiguration, ByVal kerasVersion As Integer?)
			Dim layerConfig As IDictionary(Of String, Object) = New Dictionary(Of String, Object)()
			layerConfig(conf.getLAYER_FIELD_CLASS_NAME()) = conf.getLAYER_CLASS_NAME_CROPPING_1D()
			Dim config As IDictionary(Of String, Object) = New Dictionary(Of String, Object)()
			config(conf.getLAYER_FIELD_NAME()) = LAYER_NAME
			config(conf.getLAYER_FIELD_CROPPING()) = CROPPING
			layerConfig(conf.getLAYER_FIELD_CONFIG()) = config
			layerConfig(conf.getLAYER_FIELD_KERAS_VERSION()) = kerasVersion
			Dim layer As Cropping1D = (New KerasCropping1D(layerConfig)).Cropping1DLayer
