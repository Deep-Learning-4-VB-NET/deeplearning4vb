...
'ORIGINAL LINE: public KerasGaussianNoise(System.Nullable<Integer> kerasVersion) throws org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasGaussianNoise(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasGaussianNoise(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			If Not innerConfig.ContainsKey(conf.getLAYER_FIELD_GAUSSIAN_VARIANCE()) Then
				Throw New InvalidKerasConfigurationException("Keras configuration does not contain " & conf.getLAYER_FIELD_GAUSSIAN_VARIANCE() & " parameter" & "needed for GaussianNoise")
			End If
			Dim stddev As Double = DirectCast(innerConfig(conf.getLAYER_FIELD_GAUSSIAN_VARIANCE()), Double)

			Me.layer_Conflict = (New DropoutLayer.Builder()).name(Me.layerName_Conflict).dropOut(New GaussianNoise(stddev)).build()
		End Sub
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Return Me.GaussianNoiseLayer.getOutputType(-1, inputType(0))
		End Function
		...
		Public Overridable ReadOnly Property GaussianNoiseLayer As DropoutLayer
			Get
