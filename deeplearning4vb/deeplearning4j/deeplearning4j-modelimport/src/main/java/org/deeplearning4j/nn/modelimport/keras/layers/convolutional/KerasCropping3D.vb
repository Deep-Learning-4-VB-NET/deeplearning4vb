...
'ORIGINAL LINE: @Slf4j @Data @EqualsAndHashCode(callSuper = false) public class KerasCropping3D extends org.deeplearning4j.nn.modelimport.keras.KerasLayer
	Public Class KerasCropping3D
		Inherits KerasLayer
...
'ORIGINAL LINE: public KerasCropping3D(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasCropping3D(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim croppingField As String = conf.getLAYER_FIELD_CROPPING()
			Dim cropping() As Integer = getPaddingFromConfig(layerConfig, conf, croppingField, 3)
			Dim builder As Cropping3D.Builder = (New Cropping3D.Builder(cropping)).name(Me.layerName_Conflict).dropOut(Me.dropout)
		...
		''' </summary>
		''' <returns> Cropping3D layer </returns>
		Public Overridable ReadOnly Property Cropping3DLayer As Cropping3D
			Get
				Return DirectCast(Me.layer_Conflict, Cropping3D)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			If inputType.Length > 1 Then
				Throw New InvalidKerasConfigurationException("Keras Cropping 3D layer accepts only one input (received " & inputType.Length & ")")
			End If
			Return Me.Cropping3DLayer.getOutputType(-1, inputType(0))
		End Function
	End Class
