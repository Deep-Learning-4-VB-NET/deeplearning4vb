...
'ORIGINAL LINE: @Slf4j @Data @EqualsAndHashCode(callSuper = false) public class KerasLocallyConnected2D extends org.deeplearning4j.nn.modelimport.keras.layers.convolutional.KerasConvolution
	Public Class KerasLocallyConnected2D
...
'ORIGINAL LINE: public KerasLocallyConnected2D(System.Nullable<Integer> kerasVersion) throws org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasLocallyConnected2D(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasLocallyConnected2D(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim builder As LocallyConnected2D.Builder = (New LocallyConnected2D.Builder()).name(Me.layerName_Conflict).nOut(getNOutFromConfig(layerConfig, conf)).dropOut(Me.dropout).activation(getActivationFromConfig(layerConfig, conf)).weightInit(conf.getKERAS_PARAM_NAME_W(), init).l1(Me.weightL1Regularization).l2(Me.weightL2Regularization).convolutionMode(getConvolutionModeFromConfig(layerConfig, conf)).kernelSize(getKernelSizeFromConfig(layerConfig, 2, conf, kerasMajorVersion)).hasBias(hasBias).stride(getStrideFromConfig(layerConfig, 2, conf))
		...
		''' </summary>
		''' <returns> Locally connected 2D layer. </returns>
		Public Overridable ReadOnly Property LocallyConnected2DLayer As LocallyConnected2D
			Get
				Return DirectCast(Me.layer_Conflict, LocallyConnected2D)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Dim convType As InputType.InputTypeConvolutional = DirectCast(inputType(0), InputType.InputTypeConvolutional)
			...
			DirectCast(Me.layer_Conflict, LocallyConnected2D).setInputSize(New Integer() {CInt(Math.Truncate(convType.getHeight())), CInt(Math.Truncate(convType.getWidth()))})
			DirectCast(Me.layer_Conflict, LocallyConnected2D).setNIn(convType.getChannels())
			DirectCast(Me.layer_Conflict, LocallyConnected2D).computeOutputSize()

			Dim preprocessor As InputPreProcessor = getInputPreprocessor(inputType(0))
			If preprocessor IsNot Nothing Then
				Return Me.LocallyConnected2DLayer.getOutputType(-1, preprocessor.getOutputType(inputType(0)))
			End If
			Return Me.LocallyConnected2DLayer.getOutputType(-1, inputType(0))
		End Function
