...
'ORIGINAL LINE: public KerasUpsampling1D(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasUpsampling1D(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim size() As Integer = KerasConvolutionUtils.getUpsamplingSizeFromConfig(layerConfig, 1, conf)

			Dim builder As Upsampling1D.Builder = (New Upsampling1D.Builder()).name(Me.layerName_Conflict).dropOut(Me.dropout).size(size(0))
		...
		''' </summary>
		''' <returns> Upsampling1D layer </returns>
		Public Overridable ReadOnly Property Upsampling1DLayer As Upsampling1D
			Get
				Return DirectCast(Me.layer_Conflict, Upsampling1D)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Return Me.Upsampling1DLayer.getOutputType(-1, inputType(0))
		End Function
