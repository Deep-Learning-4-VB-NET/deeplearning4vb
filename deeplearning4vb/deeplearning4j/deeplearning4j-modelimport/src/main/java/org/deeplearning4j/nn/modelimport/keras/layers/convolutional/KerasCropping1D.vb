...
'ORIGINAL LINE: @Slf4j @Data @EqualsAndHashCode(callSuper = false) public class KerasCropping1D extends org.deeplearning4j.nn.modelimport.keras.KerasLayer
...
'ORIGINAL LINE: public KerasCropping1D(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasCropping1D(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim cropping() As Integer = getPaddingFromConfig(layerConfig, conf, croppingField, 1)
			Dim builder As Cropping1D.Builder = (New Cropping1D.Builder(cropping)).name(Me.layerName_Conflict).dropOut(Me.dropout)
		...
		''' </summary>
		''' <returns> Cropping1D layer </returns>
		Public Overridable ReadOnly Property Cropping1DLayer As Cropping1D
			Get
				Return DirectCast(Me.layer_Conflict, Cropping1D)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Return Me.Cropping1DLayer.getOutputType(-1, inputType(0))
		End Function
	End Class
