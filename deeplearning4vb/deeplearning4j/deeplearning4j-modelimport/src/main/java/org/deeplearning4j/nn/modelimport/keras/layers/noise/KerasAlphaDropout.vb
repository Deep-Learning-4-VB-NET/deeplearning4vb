...
Namespace org.deeplearning4j.nn.modelimport.keras.layers.noise
	...
	''' @author Max Pumperla
	''' </summary>
	Public Class KerasAlphaDropout
		Inherits KerasLayer
...
'ORIGINAL LINE: public KerasAlphaDropout(System.Nullable<Integer> kerasVersion) throws org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasAlphaDropout(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasAlphaDropout(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			If Not innerConfig.ContainsKey(conf.getLAYER_FIELD_RATE()) Then
				Throw New InvalidKerasConfigurationException("Keras configuration does not contain " & "parameter" & conf.getLAYER_FIELD_RATE() & "needed for AlphaDropout")
			End If
			...
			Me.layer_Conflict = (New DropoutLayer.Builder()).name(Me.layerName_Conflict).dropOut(New AlphaDropout(retainRate)).build()
		End Sub
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			If inputType.Length > 1 Then
				Throw New InvalidKerasConfigurationException("Keras Alpha Dropout layer accepts only one input (received " & inputType.Length & ")")
			End If
			Return Me.AlphaDropoutLayer.getOutputType(-1, inputType(0))
		End Function
		...
		Public Overridable ReadOnly Property AlphaDropoutLayer As DropoutLayer
			Get
