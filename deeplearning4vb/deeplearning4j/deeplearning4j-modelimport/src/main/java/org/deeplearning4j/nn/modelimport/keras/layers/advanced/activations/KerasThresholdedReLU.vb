...
Imports ActivationThresholdedReLU = org.nd4j.linalg.activations.impl.ActivationThresholdedReLU
...
Namespace org.deeplearning4j.nn.modelimport.keras.layers.advanced.activations
	...
	''' @author Max Pumperla
	''' </summary>
	Public Class KerasThresholdedReLU
		Inherits KerasLayer
...
'ORIGINAL LINE: public KerasThresholdedReLU(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasThresholdedReLU(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim theta As Double = 1.0
			Dim layerFieldThresholdTheta As String = "theta"
			If innerConfig.ContainsKey(layerFieldThresholdTheta) Then
				theta = DirectCast(innerConfig(layerFieldThresholdTheta), Double)
			End If
			Dim thresholdedReLU As IActivation = New ActivationThresholdedReLU(theta)
			Me.layer_Conflict = (New ActivationLayer.Builder()).name(Me.layerName_Conflict).activation(thresholdedReLU).build()
		End Sub
