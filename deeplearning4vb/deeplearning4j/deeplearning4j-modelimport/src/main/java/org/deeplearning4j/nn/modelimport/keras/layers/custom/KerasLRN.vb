...
'ORIGINAL LINE: @Slf4j public class KerasLRN extends org.deeplearning4j.nn.modelimport.keras.KerasLayer
...
'ORIGINAL LINE: public KerasLRN(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasLRN(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim lrnParams As IDictionary(Of String, Object) = KerasLayerUtils.getInnerLayerConfigFromConfig(layerConfig, conf)

			Dim builder As LocalResponseNormalization.Builder = (New LocalResponseNormalization.Builder()).name(Me.layerName_Conflict).dropOut(Me.dropout).alpha(DirectCast(lrnParams("alpha"), Double)).beta(DirectCast(lrnParams("beta"), Double)).k(DirectCast(lrnParams("k"), Integer)).n(DirectCast(lrnParams("n"), Integer))
		...
		''' Get DL4J LRN.
		''' </summary>
		''' <returns>  LocalResponseNormalization </returns>
		Public Overridable ReadOnly Property LocalResponseNormalization As LocalResponseNormalization
			Get
				Return DirectCast(Me.layer_Conflict, LocalResponseNormalization)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Return Me.LocalResponseNormalization.getOutputType(-1, inputType(0))
		End Function
	End Class
