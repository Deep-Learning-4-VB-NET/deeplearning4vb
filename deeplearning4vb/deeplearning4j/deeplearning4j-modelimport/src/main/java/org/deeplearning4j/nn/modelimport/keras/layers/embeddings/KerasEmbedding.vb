...
'ORIGINAL LINE: @Slf4j @Data @EqualsAndHashCode(callSuper = false) public class KerasEmbedding extends org.deeplearning4j.nn.modelimport.keras.KerasLayer
...
'ORIGINAL LINE: public KerasEmbedding() throws org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New()
		End Sub
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasEmbedding(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasEmbedding(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim builder As EmbeddingSequenceLayer.Builder = (New EmbeddingSequenceLayer.Builder()).name(Me.layerName_Conflict).nIn(inputDim).inputLength(inputLength).inferInputLength(inferInputLength).nOut(nOutFromConfig).dropOut(Me.dropout).activation(Activation.IDENTITY).weightInit(init).biasInit(0.0).l1(Me.weightL1Regularization).l2(Me.weightL2Regularization).outputDataFormat(RNNFormat.NWC).hasBias(False)
		...
		Public Overridable ReadOnly Property EmbeddingLayer As EmbeddingSequenceLayer
			Get
				Return DirectCast(Me.layer_Conflict, EmbeddingSequenceLayer)
			End Get
		End Property
