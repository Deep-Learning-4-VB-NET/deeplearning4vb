...
'ORIGINAL LINE: public KerasELU(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasELU(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim alpha As Double = 1.0 ' Set default alpha to default in nd4j
			Dim layerFieldLeakyReluAlpha As String = "alpha"
			If innerConfig.ContainsKey(layerFieldLeakyReluAlpha) Then
				alpha = DirectCast(innerConfig(layerFieldLeakyReluAlpha), Double)
			End If
			Dim leakyReLU As IActivation = New ActivationELU(alpha)
			Me.layer_Conflict = (New ActivationLayer.Builder()).name(Me.layerName_Conflict).activation(leakyReLU).build()
		End Sub
