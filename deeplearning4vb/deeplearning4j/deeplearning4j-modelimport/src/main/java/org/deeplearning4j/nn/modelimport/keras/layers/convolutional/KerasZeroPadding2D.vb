...
'ORIGINAL LINE: @Slf4j @Data @EqualsAndHashCode(callSuper = false) public class KerasZeroPadding2D extends org.deeplearning4j.nn.modelimport.keras.KerasLayer
...
'ORIGINAL LINE: public KerasZeroPadding2D(java.util.Map<String, Object> layerConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		...
		''' <exception cref="UnsupportedKerasConfigurationException"> Unsupported Keras config </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public KerasZeroPadding2D(java.util.Map<String, Object> layerConfig, boolean enforceTrainingConfig) throws InvalidKerasConfigurationException, org.deeplearning4j.nn.modelimport.keras.exceptions.UnsupportedKerasConfigurationException
		Public Sub New(ByVal layerConfig As IDictionary(Of String, Object), ByVal enforceTrainingConfig As Boolean)
			...
			Dim builder As ZeroPaddingLayer.Builder = (New ZeroPaddingLayer.Builder(getPaddingFromConfig(layerConfig, conf, paddingField, 2))).dataFormat(If(dimOrder_Conflict = DimOrder.TENSORFLOW, CNN2DFormat.NHWC, CNN2DFormat.NCHW)).name(Me.layerName_Conflict).dropOut(Me.dropout)
		...
		''' </summary>
		''' <returns>  ZeroPadding2DLayer </returns>
		Public Overridable ReadOnly Property ZeroPadding2DLayer As ZeroPaddingLayer
			Get
				Return DirectCast(Me.layer_Conflict, ZeroPaddingLayer)
			End Get
		End Property
...
'ORIGINAL LINE: @Override public org.deeplearning4j.nn.conf.inputs.InputType getOutputType(org.deeplearning4j.nn.conf.inputs.InputType... inputType) throws org.deeplearning4j.nn.modelimport.keras.exceptions.InvalidKerasConfigurationException
		Public Overrides Function getOutputType(ParamArray ByVal inputType() As InputType) As InputType
			...
			End If
			Return Me.ZeroPadding2DLayer.getOutputType(-1, inputType(0))
		End Function
	End Class
