...
Namespace org.deeplearning4j.spark.data
	...
	Public Class BatchAndExportMultiDataSetsFunction
		Implements Function2(Of Integer, IEnumerator(Of MultiDataSet), IEnumerator(Of String))
...
'ORIGINAL LINE: @Override public Iterator<String> call(System.Nullable<Integer> partitionIdx, Iterator<org.nd4j.linalg.dataset.api.MultiDataSet> iterator) throws Exception
		Public Overrides Function [call](ByVal partitionIdx As Integer?, ByVal iterator As IEnumerator(Of MultiDataSet)) As IEnumerator(Of String)

			Dim outputPaths As IList(Of String) = New List(Of String)()
			Dim tempList As New LinkedList(Of MultiDataSet)()

			Dim count As Integer = 0
			Do While iterator.MoveNext()
				Dim [next] As MultiDataSet = iterator.Current
				If [next].getFeatures(0).size(0) = minibatchSize Then
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: private org.nd4j.common.primitives.Pair<Integer, List<String>> processList(LinkedList<org.nd4j.linalg.dataset.api.MultiDataSet> tempList, int partitionIdx, int countBefore, boolean finalExport) throws Exception
		Private Function processList(ByVal tempList As LinkedList(Of MultiDataSet), ByVal partitionIdx As Integer, ByVal countBefore As Integer, ByVal finalExport As Boolean) As Pair(Of Integer, IList(Of String))
			...
			For Each ds As MultiDataSet In tempList
			...
			Dim tempToMerge As IList(Of MultiDataSet) = New List(Of MultiDataSet)()
			Do While tempList.Count > 0 AndAlso countSoFar <> minibatchSize
				Dim [next] As MultiDataSet = tempList.RemoveFirst()
				If countSoFar + [next].getFeatures(0).size(0) <= minibatchSize Then
					'Add the entire DataSet object
					tempToMerge.Add([next])
					countSoFar += [next].getFeatures(0).size(0)
				Else
					'Split the DataSet
					Dim examples As IList(Of MultiDataSet) = [next].asList()
					For Each ds As MultiDataSet In examples
			...
			Dim toExport As MultiDataSet = org.nd4j.linalg.dataset.MultiDataSet.merge(tempToMerge)
'JAVA TO VB CONVERTER WARNING: An assignment within expression was extracted from the following statement:
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: private String export(org.nd4j.linalg.dataset.api.MultiDataSet dataSet, int partitionIdx, int outputCount) throws Exception
		Private Function export(ByVal dataSet As MultiDataSet, ByVal partitionIdx As Integer, ByVal outputCount As Integer) As String
			Dim filename As String = "mds_" & partitionIdx & jvmuid & "_" & outputCount & ".bin"
