...
Namespace org.deeplearning4j.spark.models.embeddings.word2vec
	...
	Public Class SecondIterationFunction
		Implements FlatMapFunction(Of IEnumerator(Of Tuple2(Of IList(Of VocabWord), Long)), KeyValuePair(Of VocabWord, INDArray))
...
'JAVA TO VB CONVERTER TODO TASK: There is no VB equivalent to 'volatile':
'ORIGINAL LINE: private transient volatile NegativeHolder negativeHolder;
'JAVA TO VB CONVERTER NOTE: The variable negativeHolder was renamed since it may cause conflicts with calls to static members of the user-defined type with this name:
		<NonSerialized>
		Private negativeHolder_Conflict As NegativeHolder
'JAVA TO VB CONVERTER TODO TASK: There is no VB equivalent to 'volatile':
'ORIGINAL LINE: private transient volatile VocabHolder vocabHolder;
		<NonSerialized>
		Private vocabHolder As VocabHolder
		...
		Public Overrides Function [call](ByVal pairIter As IEnumerator(Of Tuple2(Of IList(Of VocabWord), Long))) As IEnumerator(Of KeyValuePair(Of VocabWord, INDArray))
			Me.vocabHolder = VocabHolder.Instance
			Me.vocabHolder.setSeed(seed, vectorLength)

			If negative > 0 Then
				negativeHolder_Conflict = NegativeHolder.Instance
				negativeHolder_Conflict.initHolder(vocab, expTable, Me.vectorLength)
			End If
...
'JAVA TO VB CONVERTER WARNING: Java to VB Converter cannot determine whether both operands of this division are integer types - if they are then you should use the VB integer division operator:
				...
				Next i
			Loop
			Return vocabHolder.getSplit(vocab).GetEnumerator()
		End Function
...
'ORIGINAL LINE: final int currentWordIndex = w2.getIndex();
			...
			Dim l1 As INDArray = vocabHolder.getSyn0Vector(currentWordIndex, vocab)
			...
			Do While i < w1.getCodeLength()
				...
				End If
				' Point to
				Dim syn1 As INDArray = vocabHolder.getSyn1Vector(point)
	'            
			...
			If negative > 0 Then
				...
				Do While d < negative + 1
					...
					Else
						...
						Dim idx As Integer = CInt(Math.Abs(CInt(nextRandom.get() >> 16) Mod negativeHolder_Conflict.getTable().length()))

						target = negativeHolder_Conflict.getTable().getInt(idx)
						If target <= 0 Then
					...
					If target >= negativeHolder_Conflict.getSyn1Neg().rows() OrElse target < 0 Then
						d += 1
						Continue Do
					End If

					Dim f As Double = Nd4j.BlasWrapper.dot(l1, negativeHolder_Conflict.getSyn1Neg().slice(target))
					Dim g As Double
					If f > maxExp Then
					...
					Nd4j.BlasWrapper.level1().axpy(vectorLength, g, negativeHolder_Conflict.getSyn1Neg().slice(target), neu1e)

					Nd4j.BlasWrapper.level1().axpy(vectorLength, g, l1, negativeHolder_Conflict.getSyn1Neg().slice(target))
					d += 1
				Loop
			End If
