...
'ORIGINAL LINE: @NativeTag @Tag(TagNames.LARGE_RESOURCES) @Tag(TagNames.LONG_TEST) public class ConvDataFormatTests extends org.deeplearning4j.BaseDL4JTest
	Public Class ConvDataFormatTests
		...
		Public Shared Function params() As Stream(Of Arguments)
			...
			For Each nd4jBackend As Nd4jBackend In BaseNd4jTestWithBackends.BACKENDS
				For Each dataType As DataType In New DataType(){DataType.FLOAT, DataType.DOUBLE}
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testConv2d(org.nd4j.linalg.api.buffer.DataType dataType, org.nd4j.linalg.factory.Nd4jBackend backend)
			...
			Try
				For Each helpers As Boolean In New Boolean(){False, True}
					For Each cm As ConvolutionMode In New ConvolutionMode(){ConvolutionMode.Truncate, ConvolutionMode.Same}
						...
						Dim tc As TestCase = TestCase.builder().msg(msg).net1(getConv2dNet(dataType,CNN2DFormat.NCHW, True, cm)).net2(getConv2dNet(dataType,CNN2DFormat.NCHW, False, cm)).net3(getConv2dNet(dataType,CNN2DFormat.NHWC, True, cm)).net4(getConv2dNet(dataType,CNN2DFormat.NHWC, False, cm)).inNCHW(inNCHW).labelsNCHW(labels).labelsNHWC(labels).testLayerIdx(1).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testSubsampling2d(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testDepthwiseConv2d(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testSeparableConv2d(org.nd4j.linalg.api.buffer.DataType dataType, org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testDeconv2d(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testLRN(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
			...
			Try
				For Each helpers As Boolean In New Boolean(){False, True}
					For Each cm As ConvolutionMode In New ConvolutionMode(){ConvolutionMode.Truncate, ConvolutionMode.Same}
						...
						Dim tc As TestCase = TestCase.builder().msg(msg).net1(getLrnLayer(dataType,CNN2DFormat.NCHW, True, cm)).net2(getLrnLayer(dataType,CNN2DFormat.NCHW, False, cm)).net3(getLrnLayer(dataType,CNN2DFormat.NHWC, True, cm)).net4(getLrnLayer(dataType,CNN2DFormat.NHWC, False, cm)).inNCHW(inNCHW).labelsNCHW(labels).labelsNHWC(labels).testLayerIdx(1).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testZeroPaddingLayer(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testCropping2DLayer(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testUpsampling2d(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testBatchNormNet(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
			...
			Try
				For Each useLogStd As Boolean In New Boolean(){True, False}
					For Each helpers As Boolean In New Boolean(){False, True}
						...
						Dim tc As TestCase = TestCase.builder().msg(msg).net1(getBatchNormNet(dataType,useLogStd, CNN2DFormat.NCHW, True)).net2(getBatchNormNet(dataType,useLogStd, CNN2DFormat.NCHW, False)).net3(getBatchNormNet(dataType,useLogStd, CNN2DFormat.NHWC, True)).net4(getBatchNormNet(dataType,useLogStd, CNN2DFormat.NHWC, False)).inNCHW(inNCHW).labelsNCHW(labels).labelsNHWC(labels).testLayerIdx(1).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testCnnLossLayer(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
		Public Overridable Sub testCnnLossLayer(ByVal dataType As DataType, ByVal backend As Nd4jBackend)
			Try
				For Each helpers As Boolean In New Boolean(){False, True}
					...
					Dim tc As TestCase = TestCase.builder().msg(msg).net1(getCnnLossNet(CNN2DFormat.NCHW, True, ConvolutionMode.Same)).net2(getCnnLossNet(CNN2DFormat.NCHW, False, ConvolutionMode.Same)).net3(getCnnLossNet(CNN2DFormat.NHWC, True, ConvolutionMode.Same)).net4(getCnnLossNet(CNN2DFormat.NHWC, False, ConvolutionMode.Same)).inNCHW(inNCHW).labelsNCHW(labelsNCHW).labelsNHWC(labelsNHWC).testLayerIdx(1).nhwcOutput(True).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testSpaceToDepthNet(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
		Public Overridable Sub testSpaceToDepthNet(ByVal dataType As DataType, ByVal backend As Nd4jBackend)
			Try
				For Each helpers As Boolean In New Boolean(){False, True}
					...
					Dim tc As TestCase = TestCase.builder().msg(msg).net1(getSpaceToDepthNet(dataType,CNN2DFormat.NCHW, True)).net2(getSpaceToDepthNet(dataType,CNN2DFormat.NCHW, False)).net3(getSpaceToDepthNet(dataType,CNN2DFormat.NHWC, True)).net4(getSpaceToDepthNet(dataType,CNN2DFormat.NHWC, False)).inNCHW(inNCHW).labelsNCHW(labels).labelsNHWC(labels).testLayerIdx(1).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testSpaceToBatchNet(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testLocallyConnected(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
			...
			Try
				For Each helpers As Boolean In New Boolean(){False, True}
					For Each cm As ConvolutionMode In New ConvolutionMode(){ConvolutionMode.Truncate, ConvolutionMode.Same}
						...
						Dim tc As TestCase = TestCase.builder().msg(msg).net1(getLocallyConnectedNet(dataType,CNN2DFormat.NCHW, True, cm)).net2(getLocallyConnectedNet(dataType,CNN2DFormat.NCHW, False, cm)).net3(getLocallyConnectedNet(dataType,CNN2DFormat.NHWC, True, cm)).net4(getLocallyConnectedNet(dataType,CNN2DFormat.NHWC, False, cm)).inNCHW(inNCHW).labelsNCHW(labels).labelsNHWC(labels).testLayerIdx(1).helpers(helpers).build()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @ParameterizedTest @MethodSource("params") public void testGlobalPooling(org.nd4j.linalg.api.buffer.DataType dataType,org.nd4j.linalg.factory.Nd4jBackend backend)
		...
		Private Function getDeconv2DNet2dNet(ByVal dataType As DataType, ByVal format As CNN2DFormat, ByVal setOnLayerAlso As Boolean, ByVal cm As ConvolutionMode) As MultiLayerNetwork
			If setOnLayerAlso Then
				Return getNetWithLayer(dataType,(New Deconvolution2D.Builder()).nOut(2).activation(Activation.TANH).kernelSize(2,2).stride(2,2).build(), format, cm, Nothing)
			Else
				Return getNetWithLayer(dataType,(New Deconvolution2D.Builder()).nOut(2).activation(Activation.TANH).kernelSize(2,2).stride(2,2).build(), format, cm, Nothing)
			End If
		End Function
...
'ORIGINAL LINE: @AllArgsConstructor @Data @NoArgsConstructor @Builder private static class TestCase
		Private Class TestCase
			...
			Friend helpers As Boolean
		End Class

		Public Shared Sub testHelper(ByVal tc As TestCase)

			If Not tc.helpers Then
				Try
					CuDNNTestUtils.removeHelpers(tc.net1.Layers)
					CuDNNTestUtils.removeHelpers(tc.net2.Layers)
					CuDNNTestUtils.removeHelpers(tc.net3.Layers)
					CuDNNTestUtils.removeHelpers(tc.net4.Layers)
			...
			If Not tc.helpers Then
				Try
					CuDNNTestUtils.removeHelpers(net1a.Layers)
					CuDNNTestUtils.removeHelpers(net2a.Layers)
					CuDNNTestUtils.removeHelpers(net3a.Layers)
					CuDNNTestUtils.removeHelpers(net4a.Layers)
				Catch t As Exception
					Throw New Exception(t)
				End Try
			End If
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @EqualsAndHashCode private static class NHWCToNCHWPreprocessor implements org.deeplearning4j.nn.conf.InputPreProcessor
		<Serializable>
