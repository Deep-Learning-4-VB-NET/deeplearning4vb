...
'ORIGINAL LINE: @Slf4j public class CpuWorkspaceManager extends org.nd4j.linalg.api.memory.provider.BasicWorkspaceManager
	Public Class CpuWorkspaceManager
		...
		Protected Friend Overridable Function newWorkspace(ByVal configuration As WorkspaceConfiguration) As MemoryWorkspace
			Return If(Nd4j.WorkspaceManager.DebugMode = DebugMode.BYPASS_EVERYTHING, New DummyWorkspace(), New CpuWorkspace(configuration))
		End Function
		...
		Protected Friend Overridable Function newWorkspace(ByVal configuration As WorkspaceConfiguration, ByVal id As String) As MemoryWorkspace
			Return If(Nd4j.WorkspaceManager.DebugMode = DebugMode.BYPASS_EVERYTHING, New DummyWorkspace(), New CpuWorkspace(configuration, id))
		End Function
		...
		Protected Friend Overridable Function newWorkspace(ByVal configuration As WorkspaceConfiguration, ByVal id As String, ByVal deviceId As Integer) As MemoryWorkspace
			Return If(Nd4j.WorkspaceManager.DebugMode = DebugMode.BYPASS_EVERYTHING, New DummyWorkspace(), New CpuWorkspace(configuration, id, deviceId))
		End Function
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Override public org.nd4j.linalg.api.memory.MemoryWorkspace createNewWorkspace(@NonNull WorkspaceConfiguration configuration, @NonNull String id)
		Public Overridable Overloads Function createNewWorkspace(ByVal configuration As WorkspaceConfiguration, ByVal id As String) As MemoryWorkspace
			ensureThreadExistense()
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Override public org.nd4j.linalg.api.memory.MemoryWorkspace createNewWorkspace(@NonNull WorkspaceConfiguration configuration, @NonNull String id, System.Nullable<Integer> deviceId)
