...
'JAVA TO VB CONVERTER NOTE: The field solverAbsoluteAccuracy was renamed since Visual Basic does not allow fields to have the same name as other class members:
		...
		''' <exception cref="org.apache.commons.math3.exception.NotStrictlyPositiveException"> if {@code sd <= 0}. </exception>
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public NormalDistribution(org.nd4j.linalg.api.rng.Random rng, double mean, double sd) throws org.apache.commons.math3.exception.NotStrictlyPositiveException
...
'ORIGINAL LINE: public NormalDistribution(double mean, double sd, double inverseCumAccuracy) throws org.apache.commons.math3.exception.NotStrictlyPositiveException
		...
		''' @since 3.1 </exception>
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
'ORIGINAL LINE: public NormalDistribution(org.nd4j.linalg.api.rng.Random rng, double mean, double sd, double inverseCumAccuracy) throws org.apache.commons.math3.exception.NotStrictlyPositiveException
...
'ORIGINAL LINE: final org.nd4j.linalg.api.ndarray.INDArray ret = org.nd4j.linalg.factory.Nd4j.createUninitialized(shape, org.nd4j.linalg.factory.Nd4j.order());
		...
		Public Overrides Function sample(ByVal ret As INDArray) As INDArray
			If random.StatePointer IsNot Nothing Then
				If means IsNot Nothing Then
					Return Nd4j.Executioner.exec(New GaussianDistribution(ret, means, standardDeviation_Conflict), random)
				Else
					Return Nd4j.Executioner.exec(New GaussianDistribution(ret, mean_Conflict, standardDeviation_Conflict), random)
				End If
			Else
...
'JAVA TO VB CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
						...
						ret.putScalar(idx, standardDeviation_Conflict * random.nextGaussian() + means.getDouble(idx))
					Next i
				Else
					For i As Integer = 0 To len - 1
'JAVA TO VB CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
						ret.putScalar(idxIter.next(), standardDeviation_Conflict * random.nextGaussian() + mean_Conflict)
					Next i
				End If
				Return ret
			End If
