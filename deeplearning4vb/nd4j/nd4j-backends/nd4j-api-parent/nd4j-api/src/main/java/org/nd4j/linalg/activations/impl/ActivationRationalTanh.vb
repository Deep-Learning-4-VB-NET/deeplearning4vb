...
Imports RationalTanhBp = org.nd4j.linalg.api.ops.impl.transforms.gradient.RationalTanhBp
...
'ORIGINAL LINE: @EqualsAndHashCode(callSuper = false) @Getter public class ActivationRationalTanh extends org.nd4j.linalg.activations.BaseActivationFunction
	<Serializable>
	Public Class ActivationRationalTanh
		...
		Public Overrides Function getActivation(ByVal [in] As INDArray, ByVal training As Boolean) As INDArray
			Nd4j.Executioner.execAndReturn(New RationalTanh([in]))
			Return [in]
		End Function
		...
		Public Overrides Function backprop(ByVal [in] As INDArray, ByVal epsilon As INDArray) As Pair(Of INDArray, INDArray)
			...
			Nd4j.Executioner.execAndReturn(New RationalTanhBp([in], epsilon, [in]))
