...
Namespace org.nd4j.linalg.api.ops
	...
	Public MustInherit Class BaseReduceSameOp
...
'JAVA TO VB CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				Preconditions.checkArgument(x.dataType() = y.dataType(),"Op.X type must be the same as Op.Y type:" & " x.dataType=%s, y.dataType=%s, op=%s", x.dataType(), y.dataType(), Me.GetType().FullName)
			End If

			Dim z As INDArray = If(oc IsNot Nothing, oc.getOutputArray(0), Me.z())
			If z IsNot Nothing Then
				Preconditions.checkArgument(z.dataType() = x.dataType(), "Op.Z must be the same as Op.X type. Op.X.datatype=%s, " & "Op.Z.datatype=%s", x.dataType(), z.dataType())
			End If
		...
		Public Overrides Function calculateOutputShape(ByVal oc As OpContext) As IList(Of LongShapeDescriptor)
			...
			Dim rt As DataType = If(oc IsNot Nothing, resultType(oc), resultType())
