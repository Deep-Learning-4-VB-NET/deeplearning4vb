...
Namespace org.nd4j.linalg.api.ops.impl.transforms.custom
	...
	Public Class FakeQuantWithMinMaxVars
		...
		Public Sub New(ByVal sd As SameDiff, ByVal input As SDVariable, ByVal min As SDVariable, ByVal max As SDVariable, ByVal narrowRange As Boolean, ByVal numBits As Integer)
			MyBase.New(sd, New SDVariable(){input, min, max})
		...
		Public Sub New(ByVal x As INDArray, ByVal min As INDArray, ByVal max As INDArray, ByVal num_bits As Integer, ByVal narrow As Boolean)
			Preconditions.checkArgument(min.Vector AndAlso max.Vector AndAlso min.length() = max.length(), "FakeQuantWithMinMaxVars: min and max should be 1D tensors with the same length")
