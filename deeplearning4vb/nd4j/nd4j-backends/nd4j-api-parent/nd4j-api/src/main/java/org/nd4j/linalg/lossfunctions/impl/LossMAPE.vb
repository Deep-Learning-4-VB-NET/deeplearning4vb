...
Imports Abs = org.nd4j.linalg.api.ops.impl.transforms.same.Abs
...
'ORIGINAL LINE: @EqualsAndHashCode @JsonInclude(JsonInclude.Include.NON_NULL) @Getter public class LossMAPE implements org.nd4j.linalg.lossfunctions.ILossFunction
	<Serializable>
...
'ORIGINAL LINE: @JsonSerialize(using = org.nd4j.serde.jackson.shaded.NDArrayTextSerializer.class) @JsonDeserialize(using = org.nd4j.serde.jackson.shaded.NDArrayTextDeSerializer.class) private final org.nd4j.linalg.api.ndarray.INDArray weights;
		...
		Public Overridable Function computeGradient(ByVal labels As INDArray, ByVal preOutput As INDArray, ByVal activationFn As IActivation, ByVal mask As INDArray) As INDArray Implements ILossFunction.computeGradient
			...
			Dim actSubPredicted As INDArray = labels.sub(output)
			Dim dLda As INDArray = Nd4j.Executioner.exec(New Sign(actSubPredicted))
			Dim absLabels As INDArray = Nd4j.Executioner.exec(New Abs(labels.dup()))
			...
			Dim gradient As INDArray = activationFn.backprop(preOutput, dLda).First 'TODO activation functions with params
