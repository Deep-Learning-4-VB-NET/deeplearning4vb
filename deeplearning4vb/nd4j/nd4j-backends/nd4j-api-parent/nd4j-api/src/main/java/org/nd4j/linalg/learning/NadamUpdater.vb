...
'ORIGINAL LINE: @Data public class NadamUpdater implements GradientUpdater<org.nd4j.linalg.learning.config.Nadam>
	Public Class NadamUpdater
		Implements GradientUpdater(Of Nadam)

		Public Const M_STATE As String = "M"
		Public Const V_STATE As String = "V"

		Private config As Nadam
		Private m, v As INDArray ' moving avg & sqrd gradients

		Private gradientReshapeOrder As Char

		Public Sub New(ByVal config As Nadam)
			Me.config = config
		End Sub
...
'ORIGINAL LINE: @Override public void setState(@NonNull Map<String, org.nd4j.linalg.api.ndarray.INDArray> stateMap, boolean initialize)
		...
		Public Overridable Function getState() As IDictionary(Of String, INDArray) Implements GradientUpdater(Of Nadam).getState
		...
		Public Overridable Sub setStateViewArray(ByVal viewArray As INDArray, ByVal gradientShape() As Long, ByVal gradientOrder As Char, ByVal initialize As Boolean) Implements GradientUpdater(Of Nadam).setStateViewArray
		...
		Public Overridable Sub applyUpdater(ByVal gradient As INDArray, ByVal iteration As Integer, ByVal epoch As Integer) Implements GradientUpdater(Of Nadam).applyUpdater
			If m Is Nothing OrElse v Is Nothing Then
