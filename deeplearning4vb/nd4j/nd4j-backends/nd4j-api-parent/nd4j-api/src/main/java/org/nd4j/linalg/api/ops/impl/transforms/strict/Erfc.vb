...
'ORIGINAL LINE: @NoArgsConstructor public class Erfc extends org.nd4j.linalg.api.ops.BaseTransformStrictOp
	Public Class Erfc
		...
		Public Overrides Function doDiff(ByVal i_v As IList(Of SDVariable)) As IList(Of SDVariable)
			...
			Dim z As SDVariable = arg()
			Dim constant As SDVariable = sameDiff.onesLike(gradient).mul(-2.0 / Math.Sqrt(Math.PI))
			Dim ret As SDVariable = constant.mul(sameDiff.math().exp(z.mul(z).mul(-1))).mul(gradient)
