...
'ORIGINAL LINE: @NoArgsConstructor @Slf4j public abstract class BaseBroadcastBoolOp extends BaseOp implements BroadcastOp
	Public MustInherit Class BaseBroadcastBoolOp
		Inherits BaseOp
		...
		Public Overridable Property Dimension As Integer() Implements BroadcastOp.getDimension
			Get
				If dimension_Conflict Is Nothing Then
					If x_Conflict IsNot Nothing AndAlso y_Conflict IsNot Nothing Then
						dimension_Conflict = Shape.getBroadcastDimensions(x_Conflict.shape(), y_Conflict.shape())
					Else
...
'JAVA TO VB CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			Preconditions.checkArgument(x().dataType() = y().dataType(), "Op.X and Op.Y must have the same data type: x.dataType=%s, y.dataType=%s, op=%s", x_Conflict.dataType(), y_Conflict.dataType(), Me.GetType().FullName)

			Preconditions.checkArgument(z().B, "Op.Z must have bool type: z has type %s for op %s", z().dataType(), Me.GetType())
