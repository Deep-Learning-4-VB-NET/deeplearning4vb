...
'ORIGINAL LINE: @EqualsAndHashCode @JsonInclude(JsonInclude.Include.NON_NULL) @Getter public class LossL2 implements org.nd4j.linalg.lossfunctions.ILossFunction
	<Serializable>
	Public Class LossL2
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: public LossL2(@JsonProperty("weights") org.nd4j.linalg.api.ndarray.INDArray weights)
		...
		Protected Friend Overridable Function scoreArray(ByVal labels As INDArray, ByVal preOutput As INDArray, ByVal activationFn As IActivation, ByVal mask As INDArray) As INDArray
			...
			Dim scoreArr As INDArray = output.rsubi(labels)
		...
		Public Overridable Function computeGradient(ByVal labels As INDArray, ByVal preOutput As INDArray, ByVal activationFn As IActivation, ByVal mask As INDArray) As INDArray Implements ILossFunction.computeGradient
			...
			Dim dLda As INDArray = output.subi(labels).muli(2)
			...
			Dim gradients As INDArray = activationFn.backprop(preOutput, dLda).First 'TODO handle activation function parameter gradients
