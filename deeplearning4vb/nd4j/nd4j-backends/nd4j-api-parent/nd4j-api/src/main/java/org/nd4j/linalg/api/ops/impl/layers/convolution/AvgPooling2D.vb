...
'ORIGINAL LINE: @Slf4j @Getter @NoArgsConstructor public class AvgPooling2D extends org.nd4j.linalg.api.ops.DynamicCustomOp
	Public Class AvgPooling2D
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: public AvgPooling2D(@NonNull INDArray input, org.nd4j.linalg.api.ops.impl.layers.convolution.config.Pooling2DConfig config)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Builder(builderMethodName = "sameDiffBuilder") public AvgPooling2D(org.nd4j.autodiff.samediff.SameDiff sameDiff, org.nd4j.autodiff.samediff.SDVariable input, org.nd4j.linalg.api.ops.impl.layers.convolution.config.Pooling2DConfig config)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: public AvgPooling2D(@NonNull INDArray input, org.nd4j.linalg.api.ndarray.INDArray output, @NonNull Pooling2DConfig config)
		...
		Public Overrides Function propertiesForFunction() As IDictionary(Of String, Object)
			If config Is Nothing AndAlso iArguments.Count > 0 Then
				...
				config = Pooling2DConfig.builder().kH(iArguments(0)).kW(iArguments(1)).sH(iArguments(2)).sW(iArguments(3)).pH(iArguments(4)).pW(iArguments(5)).dH(iArguments(6)).dW(iArguments(7)).isSameMode(iArguments(8) = 1).extra(iArguments(9)).isNHWC(iArguments(10) = 1).type(Pooling2D.Pooling2DType.AVG).build()
			End If
		...
		Public Overrides Sub initFromTensorFlow(ByVal nodeDef As NodeDef, ByVal initWith As SameDiff, ByVal attributesForNode As IDictionary(Of String, AttrValue), ByVal graph As GraphDef)
			...
			Dim pooling2DConfig As Pooling2DConfig = Pooling2DConfig.builder().sH(sH).sW(sW).type(Pooling2D.Pooling2DType.AVG).isSameMode(isSameMode).kH(kH).kW(kW).pH(pH).pW(pW).isNHWC(data_format.Equals("nhwc", StringComparison.OrdinalIgnoreCase)).extra(0.0).build()
		...
		Public Overrides Sub initFromOnnx(ByVal node As Onnx.NodeProto, ByVal initWith As SameDiff, ByVal attributesForNode As IDictionary(Of String, Onnx.AttributeProto), ByVal graph As Onnx.GraphProto)
			...
			Dim padding As val = If(Not attributesForNode.ContainsKey("pads"), java.util.Arrays.asList(1L), attributesForNode("pads").getIntsList())
			Dim strides As val = attributesForNode("strides").getIntsList()

			Dim pooling2DConfig As Pooling2DConfig = Pooling2DConfig.builder().sH(strides.get(0).intValue()).sW(strides.get(1).intValue()).type(Pooling2D.Pooling2DType.AVG).isSameMode(paddingVal.equalsIgnoreCase("SAME")).kH(kernelShape.get(0).intValue()).kW(kernelShape.get(1).intValue()).pH(padding.get(0).intValue()).pW(If(padding.size() < 2, padding.get(0).intValue(), padding.get(1).intValue())).build()
			Me.config = pooling2DConfig
			addArgs()
		End Sub
