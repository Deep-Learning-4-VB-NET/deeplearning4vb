...
'ORIGINAL LINE: @Override public org.nd4j.linalg.api.ndarray.INDArray deserialize(org.nd4j.shade.jackson.core.JsonParser jsonParser, org.nd4j.shade.jackson.databind.DeserializationContext deserializationContext) throws java.io.IOException
		Public Overrides Function deserialize(ByVal jsonParser As JsonParser, ByVal deserializationContext As DeserializationContext) As INDArray
			Dim node As JsonNode = jsonParser.getCodec().readTree(jsonParser)
			If node Is Nothing Then
				Return Nothing
			End If
			Dim size As Integer = node.size()
