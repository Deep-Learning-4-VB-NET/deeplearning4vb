...
'ORIGINAL LINE: @Slf4j @Getter @NoArgsConstructor public class AvgPooling3D extends Pooling3D
	Public Class AvgPooling3D
		Inherits Pooling3D

		Public Sub New(ByVal sameDiff As SameDiff, ByVal input As SDVariable, ByVal config As Pooling3DConfig)
			MyBase.New(sameDiff, New SDVariable(){input}, Nothing, Nothing, False, config, Pooling3DType.AVG)
		End Sub

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: public AvgPooling3D(@NonNull INDArray input, org.nd4j.linalg.api.ops.impl.layers.convolution.config.Pooling3DConfig pooling3DConfig)
		Public Sub New(ByVal input As INDArray, ByVal pooling3DConfig As Pooling3DConfig)
			MyBase.New(Nothing,Nothing,New INDArray(){input},Nothing,False, pooling3DConfig, Pooling3DType.AVG)
		End Sub
