...
Namespace org.nd4j.linalg.indexing.conditions
	...
	Public Class EpsilonEquals
		...
		Private eps As Double = Nd4j.EPS_THRESHOLD
		''' <summary>
		...
		Public Sub New(ByVal value As Number)
			Me.New(value, Nd4j.EPS_THRESHOLD)
		End Sub

		Public Sub New(ByVal value As Number, ByVal eps As Number)
			MyBase.New(value)
			Me.eps = eps.doubleValue()
		End Sub
		...
		Public Overrides Function epsThreshold() As Double
			Return Me.eps
		End Function
		...
		Public Overrides Function apply(ByVal input As Number) As Boolean?
			Return Math.Abs(input.floatValue() - value_Conflict.floatValue()) < Nd4j.EPS_THRESHOLD
		End Function
	End Class
