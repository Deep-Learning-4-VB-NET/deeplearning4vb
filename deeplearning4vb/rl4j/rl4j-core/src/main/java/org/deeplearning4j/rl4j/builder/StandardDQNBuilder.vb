...
Namespace org.deeplearning4j.rl4j.builder
	...
	''' <summary>
	''' A <seealso cref="IAgentLearner"/> builder that will setup a <seealso cref="StandardDQN standard DQN"/> algorithm in addition to the setup done by <seealso cref="BaseDQNAgentLearnerBuilder"/>.
	''' </summary>
	Public Class StandardDQNBuilder
		Inherits BaseDQNAgentLearnerBuilder(Of StandardDQNBuilder.Configuration)
		...
		Protected Friend Overrides Function buildUpdateAlgorithm() As IUpdateAlgorithm(Of FeaturesLabels, StateActionRewardState(Of Integer))
			Return New StandardDQN(networks.ThreadCurrentNetwork, networks.TargetNetwork, configuration.getUpdateAlgorithmConfiguration())
		End Function
