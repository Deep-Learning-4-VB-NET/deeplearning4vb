...
'ORIGINAL LINE: @ExtendWith(MockitoExtension.class) @Tag(TagNames.FILE_IO) @NativeTag public class NonRecurrentAdvantageActorCriticTest
...
'ORIGINAL LINE: final org.nd4j.linalg.api.ndarray.INDArray data = org.nd4j.linalg.factory.Nd4j.zeros(1, 2);
			Dim data As INDArray = Nd4j.zeros(1, 2)
'JAVA TO VB CONVERTER WARNING: The original Java variable was marked 'final':
'ORIGINAL LINE: final org.deeplearning4j.rl4j.observation.Observation observation = new org.deeplearning4j.rl4j.observation.Observation(data);
		...
		Private Class ArrayListAnonymousInnerClass
			...
			Private ReadOnly outerInstance As NonRecurrentAdvantageActorCriticTest

			Private action As Integer
			Private observation As Observation

			Public Sub New(ByVal outerInstance As NonRecurrentAdvantageActorCriticTest, ByVal action As Integer, ByVal observation As Observation)
...
'ORIGINAL LINE: @Test public void when_callingCompute_expect_valueAndPolicyComputedCorrectly()
		Public Overridable Sub when_callingCompute_expect_valueAndPolicyComputedCorrectly()
			...
			result.put(CommonOutputNames.ActorCritic.Value, invocation.getArgument(0, GetType(Observation)).getData().getColumn(0).mul(-1.0))
		...
		Private Class ArrayListAnonymousInnerClass3
			...
			Public Sub New(ByVal outerInstance As NonRecurrentAdvantageActorCriticTest)
				Me.outerInstance = outerInstance
