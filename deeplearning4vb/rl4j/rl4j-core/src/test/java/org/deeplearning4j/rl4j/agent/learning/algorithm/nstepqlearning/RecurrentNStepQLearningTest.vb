...
'ORIGINAL LINE: @ExtendWith(MockitoExtension.class) @Tag(TagNames.FILE_IO) @NativeTag public class RecurrentNStepQLearningTest
...
'ORIGINAL LINE: @Mock IOutputNeuralNet targetMock;
		...
		Private Sub setup(ByVal gamma As Double)
			...
			Return result
			End Function)
			[when](targetMock.output(any(GetType(Features)))).thenAnswer(Function(invocation)
			Dim result As New NeuralNetOutput()
			result.put(CommonOutputNames.QValues, invocation.getArgument(0, GetType(Features)).get(0).mul(-2.0))
			Return result
			End Function)
...
'ORIGINAL LINE: final org.deeplearning4j.rl4j.observation.Observation observation = new org.deeplearning4j.rl4j.observation.Observation(org.nd4j.linalg.factory.Nd4j.zeros(1, 2, 1));
			Dim observation As New Observation(Nd4j.zeros(1, 2, 1))
		...
		Private Class ArrayListAnonymousInnerClass
			...
			Private ReadOnly outerInstance As RecurrentNStepQLearningTest

			Private action As Integer
			Private observation As Observation

			Public Sub New(ByVal outerInstance As RecurrentNStepQLearningTest, ByVal action As Integer, ByVal observation As Observation)
...
'JAVA TO VB CONVERTER WARNING: The original Java variable was marked 'final':
'ORIGINAL LINE: final org.deeplearning4j.rl4j.observation.Observation observation = new org.deeplearning4j.rl4j.observation.Observation(org.nd4j.linalg.factory.Nd4j.create(new double[] { -123.0, -234.0 }).reshape(1, 2, 1));
			Dim observation As New Observation(Nd4j.create(New Double() { -123.0, -234.0 }).reshape(ChrW(1), 2, 1))
			...
			Dim featuresLabels As FeaturesLabels = argument.getValue()
			assertEquals(-2.0 * observation.Data.getDouble(0, 1, 0), featuresLabels.getLabels(CommonLabelNames.QValues).getDouble(0), 0.000001)
		End Sub
...
'ORIGINAL LINE: @Test public void when_callingWithMultipleExperiences_expect_gradientsAreValid()
		...
		Private Class ArrayListAnonymousInnerClass3
			...
			Public Sub New(ByVal outerInstance As RecurrentNStepQLearningTest)
				Me.outerInstance = outerInstance

				Me.add(New StateActionReward(Of Integer)(New Observation(Nd4j.create(New Double() { -1.1, -1.2 }).reshape(ChrW(1), 2, 1)), 0, 1.0, False))
				Me.add(New StateActionReward(Of Integer)(New Observation(Nd4j.create(New Double() { -2.1, -2.2 }).reshape(ChrW(1), 2, 1)), 1, 2.0, True))
			End Sub
