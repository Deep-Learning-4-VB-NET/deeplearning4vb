...
'ORIGINAL LINE: @ExtendWith(MockitoExtension.class) @Tag(TagNames.FILE_IO) @NativeTag public class DoubleDQNTest
...
'ORIGINAL LINE: @Test public void when_isTerminal_expect_rewardValueAtIdx0()
		Public Overridable Sub when_isTerminal_expect_rewardValueAtIdx0()
			...
			Dim sut As New org.deeplearning4j.rl4j.agent.learning.algorithm.dqn.DoubleDQN(qNetworkMock, targetQNetworkMock, configuration)
		...
		Private Class ArrayListAnonymousInnerClass
			...
			Private ReadOnly outerInstance As DoubleDQNTest

			Public Sub New(ByVal outerInstance As DoubleDQNTest)
				Me.outerInstance = outerInstance

				Me.add(outerInstance.builtTransition(outerInstance.buildObservation(New Double(){1.1, 2.2}), 0, 1.0, True, outerInstance.buildObservation(New Double(){11.0, 22.0})))
			End Sub
...
'ORIGINAL LINE: @Test public void when_isNotTerminal_expect_rewardPlusEstimatedQValue()
		Public Overridable Sub when_isNotTerminal_expect_rewardPlusEstimatedQValue()
			...
			[when](targetQNetworkMock.output(any(GetType(Features)))).thenAnswer(Function(i)
			Dim result As New NeuralNetOutput()
			result.put(CommonOutputNames.QValues, i.getArgument(0, GetType(Features)).get(0).mul(-1.0))
			Return result
			End Function)

			Dim stateActionRewardStates As IList(Of StateActionRewardState(Of Integer)) = New ArrayListAnonymousInnerClass2(Me)

			Dim sut As New DoubleDQN(qNetworkMock, targetQNetworkMock, configuration)
		...
		Private Class ArrayListAnonymousInnerClass2
			...
			Public Sub New(ByVal outerInstance As DoubleDQNTest)
				...
				Me.add(outerInstance.builtTransition(outerInstance.buildObservation(New Double(){1.1, 2.2}), 0, 1.0, False, outerInstance.buildObservation(New Double(){11.0, 22.0})))
			End Sub
...
'ORIGINAL LINE: @Test public void when_batchHasMoreThanOne_expect_everySampleEvaluated()
		...
		Private Class ArrayListAnonymousInnerClass3
			...
			Public Sub New(ByVal outerInstance As DoubleDQNTest)
				...
				Me.add(outerInstance.builtTransition(outerInstance.buildObservation(New Double(){1.1, 2.2}), 0, 1.0, False, outerInstance.buildObservation(New Double(){11.0, 22.0})))
				Me.add(outerInstance.builtTransition(outerInstance.buildObservation(New Double(){3.3, 4.4}), 1, 2.0, False, outerInstance.buildObservation(New Double(){33.0, 44.0})))
				Me.add(outerInstance.builtTransition(outerInstance.buildObservation(New Double(){5.5, 6.6}), 0, 3.0, True, outerInstance.buildObservation(New Double(){55.0, 66.0})))
			End Sub
		...
		Private Function builtTransition(ByVal observation As Observation, ByVal action As Integer?, ByVal reward As Double, ByVal isTerminal As Boolean, ByVal nextObservation As Observation) As StateActionRewardState(Of Integer)
