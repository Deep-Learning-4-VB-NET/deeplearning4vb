...
'ORIGINAL LINE: @ExtendWith(MockitoExtension.class) @Tag(TagNames.FILE_IO) @NativeTag public class MultiLayerNetworkHandlerTest
	Public Class MultiLayerNetworkHandlerTest

		Private Const LABEL_NAME As String = "TEST_LABEL"
		Private Const GRADIENT_NAME As String = "TEST_GRADIENT"

		Private modelMock As MultiLayerNetwork
		Private trainingListenerMock As TrainingListener
		Private configurationMock As MultiLayerConfiguration

		Private sut As MultiLayerNetworkHandler

		Public Overridable Sub setup(ByVal setupRecurrent As Boolean)
			modelMock = mock(GetType(MultiLayerNetwork))
			trainingListenerMock = mock(GetType(TrainingListener))

			configurationMock = mock(GetType(MultiLayerConfiguration))
			[when](configurationMock.getIterationCount()).thenReturn(123)
			[when](configurationMock.EpochCount).thenReturn(234)
			[when](modelMock.LayerWiseConfigurations).thenReturn(configurationMock)

			If setupRecurrent Then
				[when](modelMock.OutputLayer).thenReturn(New RnnOutputLayer(Nothing, Nothing))
			End If
...
'ORIGINAL LINE: final java.util.Collection<org.deeplearning4j.optimize.api.TrainingListener> listeners = new java.util.ArrayList<org.deeplearning4j.optimize.api.TrainingListener>()
		...
		Private Class ArrayListAnonymousInnerClass
			...
			Private ReadOnly outerInstance As MultiLayerNetworkHandlerTest

			Public Sub New(ByVal outerInstance As MultiLayerNetworkHandlerTest)
...
'ORIGINAL LINE: @Test public void when_callingPerformFit_expect_fitCalledOnModelWithCorrectLabels()
		Public Overridable Sub when_callingPerformFit_expect_fitCalledOnModelWithCorrectLabels()
			...
			Dim featuresArg As INDArray = featuresCaptor.getValue()
...
'ORIGINAL LINE: @Test public void when_callingperformGradientsComputation_expect_modelCalledWithCorrectFeaturesLabels()
		Public Overridable Sub when_callingperformGradientsComputation_expect_modelCalledWithCorrectFeaturesLabels()
			...
			Dim debug As Object = labelsCaptor.getAllValues()
			Dim labelsArg As INDArray = labelsCaptor.getValue()
...
'ORIGINAL LINE: final org.deeplearning4j.nn.gradient.Gradient gradient = mock(org.deeplearning4j.nn.gradient.Gradient.class);
			...
			Dim updaterMock As Updater = mock(GetType(Updater))
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Test public void when_callingRecurrentStepOutput_expect_recurrentStepCalledWithObservation()
		Public Overridable Sub when_callingRecurrentStepOutput_expect_recurrentStepCalledWithObservation()
			' Arrange
			setup(False)
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Public Overridable Sub when_callingClone_expect_handlerAndModelIsCloned()
			...
			' Act
			Dim result As MultiLayerNetworkHandler = DirectCast(sut.clone(), MultiLayerNetworkHandler)
			...
			Dim privateField As System.Reflection.FieldInfo = GetType(MultiLayerNetworkHandler).getDeclaredField("labelName")
			privateField.setAccessible(True)
			Dim cloneLabelNames As String = CStr(privateField.get(sut))
			assertEquals(cloneLabelNames, LABEL_NAME)

			privateField = GetType(MultiLayerNetworkHandler).getDeclaredField("gradientName")
			privateField.setAccessible(True)
...
'ORIGINAL LINE: @Test public void when_callingCopyFrom_expect_modelParamsAreCopiedToModel()
		Public Overridable Sub when_callingCopyFrom_expect_modelParamsAreCopiedToModel()
			...
			Dim from As New MultiLayerNetworkHandler(modelMock, Nothing, Nothing, 0)
