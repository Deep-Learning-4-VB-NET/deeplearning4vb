...
'ORIGINAL LINE: @Tag(TagNames.FILE_IO) @Tag(TagNames.JAVA_ONLY) @Tag(TagNames.SPARK) @Tag(TagNames.DIST_SYSTEMS) public class TestCalculateSortedRank extends org.datavec.spark.BaseSparkTest
	<Serializable>
...
'ORIGINAL LINE: @Test public void testCalculateSortedRank()
		Public Overridable Sub testCalculateSortedRank()
			...
			data.Add(New List(Of Writable) From {
				New Text("3"),
				New DoubleWritable(0.3)
			})
			data.Add(New List(Of Writable) From {
				New Text("2"),
				New DoubleWritable(0.2)
			})
			data.Add(New List(Of Writable) From {
				New Text("1"),
			...
			Dim [out] As JavaRDD(Of IList(Of Writable)) = SparkTransformExecutor.execute(rdd, tp)

			Dim collected As IList(Of IList(Of Writable)) = [out].collect()
