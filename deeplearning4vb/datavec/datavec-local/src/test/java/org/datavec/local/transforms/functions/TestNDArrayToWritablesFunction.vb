...
Imports NDArrayToWritablesFunction = org.datavec.local.transforms.misc.NDArrayToWritablesFunction
...
'ORIGINAL LINE: @Tag(TagNames.FILE_IO) @NativeTag public class TestNDArrayToWritablesFunction
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Public Overridable Sub testNDArrayToWritablesScalars()
			...
			Next i
			Dim actual As IList(Of Writable) = (New NDArrayToWritablesFunction()).apply(arr)
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Public Overridable Sub testNDArrayToWritablesArray()
			...
			Dim actual As IList(Of Writable) = (New NDArrayToWritablesFunction(True)).apply(arr)
			assertEquals(expected, actual)
		End Sub
	End Class
