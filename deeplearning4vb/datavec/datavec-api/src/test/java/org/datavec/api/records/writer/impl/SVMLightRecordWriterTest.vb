...
Imports SVMLightRecordWriter = org.datavec.api.records.writer.impl.misc.SVMLightRecordWriter
...
'ORIGINAL LINE: @DisplayName("Svm Light Record Writer Test") @Tag(TagNames.JAVA_ONLY) @Tag(TagNames.FILE_IO) class SVMLightRecordWriterTest extends org.nd4j.common.tests.BaseND4JTest
	Friend Class SVMLightRecordWriterTest
...
'JAVA TO VB CONVERTER WARNING: Method 'throws' clauses are not available in VB:
		Friend Overridable Sub testNoLabel()
			...
			configReader.setInt(SVMLightRecordReader.NUM_FEATURES, 10)
			configReader.setBoolean(SVMLightRecordReader.ZERO_BASED_INDEXING, False)
			Dim inputFile As File = (New ClassPathResource("datavec-api/svmlight/noLabels.txt")).File
			executeTest(configWriter, configReader, inputFile)
		End Sub
...
'ORIGINAL LINE: public static void executeTest(org.datavec.api.conf.Configuration configWriter, org.datavec.api.conf.Configuration configReader, java.io.File inputFile) throws Exception
		Public Shared Sub executeTest(ByVal configWriter As Configuration, ByVal configReader As Configuration, ByVal inputFile As File)
			Dim tempFile As File = File.createTempFile("SVMLightRecordWriter", ".txt")
			tempFile.setWritable(True)
			tempFile.deleteOnExit()
			If tempFile.exists() Then
				tempFile.delete()
			End If
			Using writer As New org.datavec.api.records.writer.impl.misc.SVMLightRecordWriter()
				...
				Loop
			End Using
			Dim p As Pattern = Pattern.compile(String.Format("{0}:\d+ ", SVMLightRecordReader.QID_PREFIX))
			Dim linesOriginal As IList(Of String) = New List(Of String)()
			For Each line As String In FileUtils.readLines(inputFile)
				If Not line.StartsWith(SVMLightRecordReader.COMMENT_CHAR, StringComparison.Ordinal) Then
					Dim lineClean As String = line.Split(SVMLightRecordReader.COMMENT_CHAR, 2)(0)
