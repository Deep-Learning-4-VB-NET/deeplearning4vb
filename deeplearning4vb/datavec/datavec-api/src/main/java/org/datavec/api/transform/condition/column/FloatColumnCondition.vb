...
'ORIGINAL LINE: @EqualsAndHashCode(callSuper = true) @Data public class FloatColumnCondition extends BaseColumnCondition
	<Serializable>
	Public Class FloatColumnCondition
		...
		Private ReadOnly set As ISet(Of Single)
		...
		Public Sub New(ByVal columnName As String, ByVal op As ConditionOp, ByVal value As Single)
		...
		''' <param name="op">                    Operation (<, >=, !=, etc) </param>
		''' <param name="value">                 Value to use in the condition </param>
		Public Sub New(ByVal column As String, ByVal sequenceConditionMode As SequenceConditionMode, ByVal op As ConditionOp, ByVal value As Single)
		...
		Public Sub New(ByVal column As String, ByVal op As ConditionOp, ByVal set As ISet(Of Single))
		...
		''' <param name="op">                    Operation. Must be either ConditionOp.InSet, ConditionOp.NotInSet </param>
		''' <param name="set">                   Set to use in the condition </param>
		Public Sub New(ByVal column As String, ByVal sequenceConditionMode As SequenceConditionMode, ByVal op As ConditionOp, ByVal set As ISet(Of Single))
			MyBase.New(column, sequenceConditionMode)
...
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: private FloatColumnCondition(@JsonProperty("columnName") String columnName, @JsonProperty("op") org.datavec.api.transform.condition.ConditionOp op, @JsonProperty("value") float value, @JsonProperty("set") java.util.@Set<Single> set)
		Private Sub New(ByVal columnName As String, ByVal op As ConditionOp, ByVal value As Single, ByVal set As ISet(Of Single))
		...
		Public Overrides Function columnCondition(ByVal writable As Writable) As Boolean
			Return op.apply(writable.toFloat(), (If(value Is Nothing, Float.NaN, value)), set)
		End Function

		Public Overrides Function ToString() As String
			Return "FloatColumnCondition(columnName=""" & columnName_Conflict & """," & op & "," & (If(op = ConditionOp.NotInSet OrElse op = ConditionOp.InSet, set, value)) & ")"
		End Function
		...
		Public Overrides Function condition(ByVal input As Object) As Boolean
			...
			Return op.apply(d.floatValue(), (If(value Is Nothing, Float.NaN, value)), set)
		End Function
