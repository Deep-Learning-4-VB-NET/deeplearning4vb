...
'ORIGINAL LINE: @AllArgsConstructor @Data public class FloatWritableOp<T> implements IAggregableReduceOp<org.datavec.api.writable.Writable, T>
	<Serializable>
	Public Class FloatWritableOp(Of T)
		Implements IAggregableReduceOp(Of Writable, T)

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Getter private IAggregableReduceOp<Single, T> operation;
		Private operation As IAggregableReduceOp(Of Single, T)
		...
		Public Overridable Sub combine(Of W As IAggregableReduceOp(Of Writable, T))(ByVal accu As W) Implements IAggregableReduceOp(Of Writable, T).combine
			If TypeOf accu Is FloatWritableOp Then
				operation.combine(CType(accu, FloatWritableOp).getOperation())
			Else
