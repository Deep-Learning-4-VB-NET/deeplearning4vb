...
'ORIGINAL LINE: @AllArgsConstructor @Data public class LongWritableOp<T> implements IAggregableReduceOp<org.datavec.api.writable.Writable, T>
	<Serializable>
	Public Class LongWritableOp(Of T)
		Implements IAggregableReduceOp(Of Writable, T)

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Getter private IAggregableReduceOp<Long, T> operation;
		Private operation As IAggregableReduceOp(Of Long, T)
		...
		Public Overridable Sub combine(Of W As IAggregableReduceOp(Of Writable, T))(ByVal accu As W) Implements IAggregableReduceOp(Of Writable, T).combine
			If TypeOf accu Is LongWritableOp Then
				operation.combine(CType(accu, LongWritableOp).getOperation())
			Else
