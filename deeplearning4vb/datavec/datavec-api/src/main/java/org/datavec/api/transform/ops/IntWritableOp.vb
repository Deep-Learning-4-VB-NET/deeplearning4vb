...
'ORIGINAL LINE: @AllArgsConstructor @Data public class IntWritableOp<T> implements IAggregableReduceOp<org.datavec.api.writable.Writable, T>
	<Serializable>
	Public Class IntWritableOp(Of T)
		Implements IAggregableReduceOp(Of Writable, T)

'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: @Getter private IAggregableReduceOp<Integer, T> operation;
		Private operation As IAggregableReduceOp(Of Integer, T)
		...
		Public Overridable Sub combine(Of W As IAggregableReduceOp(Of Writable, T))(ByVal accu As W) Implements IAggregableReduceOp(Of Writable, T).combine
			If TypeOf accu Is IntWritableOp Then
				operation.combine(CType(accu, IntWritableOp).getOperation())
			Else
