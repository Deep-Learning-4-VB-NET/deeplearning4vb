...
'ORIGINAL LINE: @EqualsAndHashCode(callSuper = true) @Data public class CategoricalColumnCondition extends BaseColumnCondition
	<Serializable>
	Public Class CategoricalColumnCondition
		...
		Private ReadOnly set As ISet(Of String)
		...
		''' <param name="op">         Operation (== or != only) </param>
		''' <param name="value">      Value to use in the condition </param>
		Public Sub New(ByVal columnName As String, ByVal op As ConditionOp, ByVal value As String)
		...
		''' <param name="op">                    Operation (== or != only) </param>
		''' <param name="value">                 Value to use in the condition </param>
		Public Sub New(ByVal columnName As String, ByVal sequenceConditionMode As SequenceConditionMode, ByVal op As ConditionOp, ByVal value As String)
			MyBase.New(columnName, sequenceConditionMode)
			If op <> ConditionOp.Equal AndAlso op <> ConditionOp.NotEqual Then
				Throw New System.ArgumentException("Invalid condition op: can only use this constructor with Equal or NotEqual conditions")
			End If
			Me.op = op
			Me.value = value
			Me.set = Nothing
		...
		Public Sub New(ByVal columnName As String, ByVal op As ConditionOp, ByVal set As ISet(Of String))
			Me.New(columnName, DEFAULT_SEQUENCE_CONDITION_MODE, op, set)
		End Sub

		'Private constructor for Jackson deserialization only
'JAVA TO VB CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
'ORIGINAL LINE: private CategoricalColumnCondition(@JsonProperty("columnName") String columnName, @JsonProperty("op") org.datavec.api.transform.condition.ConditionOp op, @JsonProperty("value") String value, @JsonProperty("set") java.util.@Set<String> set)
		Private Sub New(ByVal columnName As String, ByVal op As ConditionOp, ByVal value As String, ByVal set As ISet(Of String))
		...
		''' <param name="op">                    Operation. Must be either ConditionOp.InSet, ConditionOp.NotInSet </param>
		''' <param name="set">                   Set to use in the condition </param>
		Public Sub New(ByVal columnName As String, ByVal sequenceConditionMode As SequenceConditionMode, ByVal op As ConditionOp, ByVal set As ISet(Of String))
			MyBase.New(columnName, sequenceConditionMode)
		...
		Public Overrides Function ToString() As String
			Return "CategoricalColumnCondition(columnName=""" & columnName_Conflict & """," & op & "," & (If(op = ConditionOp.NotInSet OrElse op = ConditionOp.InSet, set, value)) & ")"
		End Function
